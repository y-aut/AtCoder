...
void solve() {
    ...
    rep(b, 2) {
        ...
        rep(i, N) rep(j, N) {
            if ((i + j) % 2 != b) continue;
            vll ary;
            if (i != 0) ary.pb(A[i - 1][j]);
            if (i != N - 1) ary.pb(A[i + 1][j]);
            if (j != 0) ary.pb(A[i][j - 1]);
            if (j != N - 1) ary.pb(A[i][j + 1]);
            sort(all(ary));
            bool ok = true;
            repi(k, ary) if (abs(A[i][j] - k) <= d) BREAK(ok = false);
            if (ok) continue;
            ll m_val = ary[0] - d, m = abs(A[i][j] - m_val);
            rep(k, ary.size() - 1) {
                if (ary[k + 1] - ary[k] >= 2 * d) {
                    ll m2_val = ary[k], m2 = abs(A[i][j] - m2_val);
                    if (m2 < m) {
                        m = m2;
                        m_val = m2_val;
                    }
                    m2_val = ary[k + 1], m2 = abs(A[i][j] - m2_val);
                    if (m2 < m) {
                        m = m2;
                        m_val = m2_val;
                    }
            ...
            ll m2_val = ary.back() + d, m2 = abs(A[i][j] - m2_val);
            if (m2 < m) {
                m = m2;
                m_val = m2_val;
            }
