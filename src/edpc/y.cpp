...
int main() {
    INLL(H, W, N);
    auto walls = in_vpll(N);
    repi(i, walls) {
        i.first--;
        i.second--;
    }
    sort(all(walls), [](pll &a, pll &b) { return a.first == b.first ? a.second < b.second : a.first < b.first; });

    Binomial bin(H + W, MOD);
    mint dp[2][3010] = {};

    rep(i, N) {
        dp[1][i] = bin.nCk(walls[i].first + walls[i].second, walls[i].first);
        rep(j, i) {
            ll dif = walls[i].second - walls[j].second;
            if (dif >= 0) {
                rep(p, 2) dp[p][i] += dp[p ^ 1][j] * bin.nCk(walls[i].first - walls[j].first + dif, dif);
            }
        }
    }

    mint ans = bin.nCk(H + W - 2, H - 1);
    rep(i, N) {
        ans += dp[0][i] * bin.nCk(H - 1 - walls[i].first + W - 1 - walls[i].second, H - 1 - walls[i].first);
        ans -= dp[1][i] * bin.nCk(H - 1 - walls[i].first + W - 1 - walls[i].second, H - 1 - walls[i].first);
    }
    print(ans);
