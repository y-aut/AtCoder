...
int main() {
    INLL(N);
    auto wsv = in_vvll(3, N);

    sort(all(wsv), [](vll &a, vll &b) { return a[0] + a[1] < b[0] + b[1]; });
    ...
    rep(i, N) {
        auto nxt = dp;
        chmax(nxt[wsv[i][0]], wsv[i][2]);
        reps(j, 1, wsv[i][1] + 1) {
            if (dp[j]) {
                if (j + wsv[i][0] > 10000) {
                    chmax(ans, dp[j] + wsv[i][2]);
                } else {
                    chmax(nxt[j + wsv[i][0]], dp[j] + wsv[i][2]);
                }
            }
        }
        dp = nxt;
