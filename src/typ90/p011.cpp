...
int main() {
    const auto N = in_int();
    auto DCS = in_vvll(3, N);

    sort(DCS.begin(), DCS.end(), [](vll a, vll b) -> bool {
        return a.at(0) < b.at(0);
    });
    ll length = DCS.back().at(0) + 1;

    // dp[i][j]: j 日で task[i-1] まで見終わっているときの報酬の最大値
    auto dp = new ll*[N + 1];
    rep(i, N + 1) {
        dp[i] = new ll[length];
    }
    memset(dp[0], 0, sizeof(ll) * length);

    rrep(i, N) {
        rep(j, length) {
            if (j < DCS[i-1].at(1) || j > DCS[i-1].at(0)) {
                dp[i][j] = dp[i-1][j];
            }
            else {
                dp[i][j] = max(dp[i-1][j], dp[i-1][j-DCS[i-1].at(1)] + DCS[i-1].at(2));
            }
        }
    }

    ll ans = *max_element(dp[N], dp[N] + length);
    print(ans);
