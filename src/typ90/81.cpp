...
int main() {
    ...
    sort(all(AB), [](pll &a, pll &b) { return a.first < b.first; });

    auto hd = AB.begin();
    auto tl = upper_bound(all(AB), (pll){AB[0].first + K, 0}, [](const pll &a, const pll &b) { return a.first < b.first; });

    // 体重が [i, i+K] の者の人数
    lazy_segtree<ll, op, e, ll, add, add, e> tree(5010);
    for (auto i = hd; i != tl; i++)
        tree.apply(max(i->second - K, 1ll), i->second + 1, 1);

    ll ans = tree.all_prod();

    while (tl != AB.end()) {
        while (hd->first + K < tl->first) {
            tree.apply(max(hd->second - K, 1ll), hd->second + 1, -1);
            hd++;
        }
        tree.apply(max(tl->second - K, 1ll), tl->second + 1, 1);
        tl++;
        chmax(ans, tree.all_prod());
    }
