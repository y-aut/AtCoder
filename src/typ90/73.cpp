...
modint1000000007 dp[100010][3];

void dfs(vc &c, vvll &edges, ll v, ll last) {
    dp[v][c[v]] = 1;
    repi(i, edges[v]) {
        if (i == last)
            continue;
        dfs(c, edges, i, v);
        auto one = dp[v][c[v]] * (dp[i][c[v]] + dp[i][2]);
        auto both = dp[v][c[v]] * (dp[i][1 - c[v]] + dp[i][2]) + dp[v][2] * (dp[i][0] + dp[i][1] + dp[i][2] * 2);
        dp[v][c[v]] = one;
        dp[v][2] = both;
    }
}
