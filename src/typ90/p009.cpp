...
double get_arg2(double t1, double t2) {
	auto dif = abs(t1 - t2);
	return dif > M_PI ? M_PI * 2 - dif : dif;
}
...
int main()
{
	...
	rep(i, N) {
		auto p = XY.at(i);
		auto args = vector<double>();
		rep(j, N) {
			if (i == j) continue;
			auto q = XY.at(j);
			args.pb(atan2(q.at(1)-p.at(1), q.at(0)-p.at(0)));
		}
		sort(args.begin(), args.end());
		for (auto arg : args) {
			auto target = arg + M_PI;
			if (target >= M_PI * 2) target -= M_PI * 2;
			auto index = distance(args.begin(), lower_bound(args.begin(), args.end(), target)) % (N - 1);
			auto index_1 = index == 0 ? N - 2 : index - 1;
			max_ang = max(max_ang, max(get_arg2(args.at(index_1), arg), get_arg2(args.at(index), arg)));
		}
	}

	cout << setprecision(20) << (max_ang * 180 / M_PI) << endl;
