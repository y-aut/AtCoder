...
int main() {
    ...
    if (groups.size() >= 4) {
        ...
        rep(i, K) {
            ...
            repi(j, gis) {
                if (j == 3) {
                    reps(k, 3, sorted_groups.size()) repi(l, sorted_groups[k]) ans.pb(l);
                } else {
                    repi(k, sorted_groups[j]) ans.pb(k);
                }
            }
            print1(ans);
