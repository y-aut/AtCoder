...
public:
    vector<string> fullJustify(vector<string> &words, int maxWidth) {
        ...
        for (int i = 1; i < words.size(); i++) {
            ...
            if (i == cur_start_i + 1) {
                ans.push_back(words[cur_start_i] + string(maxWidth - cur_len, ' '));
            } else {
                int space = 1 + (maxWidth - cur_len) / (i - cur_start_i - 1);
                int surplus = (maxWidth - cur_len) % (i - cur_start_i - 1);
                ans.push_back("");
                for (int j = cur_start_i; j < i; j++) {
                    ans.back() += words[j];
                    if (j != i - 1) {
                        ans.back() += string(surplus-- > 0 ? space + 1 : space, ' ');
                    }
                }
            }
        ...
        ans.push_back("");
        for (int j = cur_start_i; j < words.size(); j++) {
            ans.back() += words[j];
            if (j != words.size() - 1) {
                ans.back() += string(1, ' ');
            }
