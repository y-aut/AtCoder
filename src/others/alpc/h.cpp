...
void solve() {
    ...
    VPLL(XY, N);
    vll P;
    repi(i, XY) P.pb(i.first);
    repi(i, XY) P.pb(i.second);
    scc_graph g(N * 2);
    rep(i, N * 2) rep(j, i + 1, N * 2) {
        if (j - i == N) continue;
        if (abs(P[i] - P[j]) < D) {
            g.add_edge(i, j < N ? j + N : j - N);
            g.add_edge(j, i < N ? i + N : i - N);
        }
    }
    auto scc = g.scc();
    vll ans(N, -1);
    repi(i, scc) {
        bool val = false;
        repi(j, i) {
            if (ans[j % N] == -1) continue;
            val = j >= N ^ ans[j % N];
            break;
        }
        repi(j, i) {
            if (ans[j % N] != -1 && (j >= N ^ ans[j % N]) != val) {
                No;
                return;
            }
            ans[j % N] = (j >= N) ^ val;
    ...
    Yes;
    rep(i, N) {
        print(ans[i] <= 0 ? XY[i].second : XY[i].first);
    }
}
