...
S e() { return {0, 0}; }
S mp(F f, S x) { return {f.first * x.first + f.second * x.second, x.second}; }
F comp(F f, F g) { return {f.first * g.first, f.first * g.second + f.second}; }
F id() { return {1, 0}; }
...
void solve() {
    ...
    VLL(a, N);
    vector<S> init;
    repi(i, a) init.eb(i, 1);
    lazy_segtree<S, op, e, F, mp, comp, id> tree(init);
    rep(q, Q) {
        LL(op, l, r);
        if (op == 0) {
            LL(b, c);
