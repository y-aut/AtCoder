...
dat mp(bool f, dat x) {
    if (f) {
        x.inv = (x.zeros + x.ones) * (x.zeros + x.ones - 1) / 2 - x.zeros * (x.zeros - 1) / 2 - x.ones * (x.ones - 1) / 2 - x.inv;
        swap(x.zeros, x.ones);
    }
    return x;
}
