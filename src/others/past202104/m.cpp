...
ll index(const pll &interval) { return interval.first * 10000000000 + interval.second; }

void insert(umll &cnt, ll &ans, IntervalSet<false> &s, umll &num, pll interval, ll n) {
    s.insert(interval);
    num[index(interval)] = n;
    ans -= cnt[n] * (cnt[n] - 1) / 2;
    cnt[n] += interval.second - interval.first;
    ans += cnt[n] * (cnt[n] - 1) / 2;
}

void erase(umll &cnt, ll &ans, IntervalSet<false> &s, umll &num, pll interval) {
    s.get_set().erase(interval);
    auto n = num[index(interval)];
    num.erase(index(interval));
    ans -= cnt[n] * (cnt[n] - 1) / 2;
    cnt[n] -= interval.second - interval.first;
    ans += cnt[n] * (cnt[n] - 1) / 2;
}
...
void solve() {
    ...
    umll num;
    rep(i, N) {
        insert(cnt, ans, s, num, {i, i + 1}, A[i]);
    }

    rep(q, Q) {
        LL(l, r, x);
        l--;
        auto v = s.intersect({l, r});
        repi(i, v) {
            auto n = num[index(i)];
            erase(cnt, ans, s, num, i);

            if (i.first < l) {
                insert(cnt, ans, s, num, {i.first, l}, n);
                if (i.second > r) {
                    insert(cnt, ans, s, num, {r, i.second}, n);
                }
            } else if (i.second > r) {
                insert(cnt, ans, s, num, {r, i.second}, n);
            }
        }
        insert(cnt, ans, s, num, {l, r}, x);
        print(ans);
    }
}
