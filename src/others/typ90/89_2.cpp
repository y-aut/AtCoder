...
int main() {
    ...
    ll max_a = -1;
    {
        ...
        auto B = A;
        sort(all(B));
        auto itr = unique(all(B));
        um<ll, ll> idx;
        ll cnt = 0;
        for (auto i = B.begin(); i != itr; i++) {
            idx[*i] = cnt++;
        }
        rep(i, N) {
            chmax(max_a, A[i] = idx[A[i]]);
        }
    }

    fenwick_tree<ll> tree(max_a + 1);

    // pos[i]: [j - 1, i] > K, [j, i] <= K なる j
    vll pos(N);
    pos[0] = 0;
    tree.add(A[0], 1);
    ...
    ll cur = 0;
    reps(i, 1, N) {
        // [pos[i - 1], i - 1] に含まれる A[i] より大きい要素の数
        cur += tree.sum(A[i] + 1, max_a + 1);
        tree.add(A[i], 1);
        auto p = pos[i - 1];
        while (cur > K) {
            cur -= tree.sum(0, A[p]);
    ...
    // dp[i]: A[i] までの分割の通り数 (直前で分割したもの, してないもの)
    vector<pair<modint1000000007, modint1000000007>> dp(N);
    dp[0] = {1, 0};
    reps(i, 1, N) {
        auto tmp = dp[i - 1].first + dp[i - 1].second;
        modint1000000007 cnt = 0;
        reps(j, pos[i - 1], pos[i]) {
            cnt += dp[j].first;
        }
        dp[i] = {tmp, tmp - cnt};
    }
