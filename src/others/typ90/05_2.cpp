...
void solve() {
    ...
    // dp[i+1][(j*10+c_k) % B] += dp[i][j] for c_k
    um<ll, vm> res;
    auto f = [&](auto rc, ll n) -> void {
        ...
        if (n & 1) {
            rc(rc, n - 1);
            rep(i, B) rep(j, K) {
                res[n][(i * 10 + c[j]) % B] += res[n - 1][i];
            }
            pow10[n] = (pow10[n - 1] * 10) % B;
        } else {
            rc(rc, n >> 1);
            pow10[n] = (pow10[n >> 1] * pow10[n >> 1]) % B;
            rep(i, B) rep(j, B) {
                res[n][(i * pow10[n >> 1] + j) % B] += res[n >> 1][i] * res[n >> 1][j];
            }
        }
    };
