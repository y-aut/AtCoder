...
int dfs(int H, int W, vs &c, pii start, pii now, usi &visited) {
    int ans = 0;
    repi(i, dir) {
        pii v = {now.first + i.first, now.second + i.second};
        if (!(0 <= v.first && v.first < H && 0 <= v.second && v.second < W))
            continue;
        if (c[v.first][v.second] == '#')
            continue;
        auto idx = v.first * W + v.second;
        if (visited.find(idx) != visited.end())
            continue;
        if (v == start) {
            if (visited.size() >= 2) {
                chmax(ans, (int)visited.size() + 1);
            }
            continue;
        }
        visited.insert(idx);
        auto res = dfs(H, W, c, start, v, visited);
        chmax(ans, res);
        visited.erase(idx);
    }
    return ans;
...
int main() {
    ...
    usi visited;
    rep(i, H) rep(j, W) {
        auto res = dfs(H, W, c, {i, j}, {i, j}, visited);
        chmax(ans, res);
    }
