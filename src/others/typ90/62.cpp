...
void bfs(vvll &graph, vll &start, vll &order) {
    vb visited = vb(graph.size(), false);
    queue<ll> q;
    repi(i, start) {
        q.emplace(i);
        visited[i] = true;
...
int main() {
    ...
    rep(i, N) {
        edges[AB[i].first].pb(i);
        edges[AB[i].second].pb(i);
        if (AB[i].first == i || AB[i].second == i) {
            start.pb(i);
        }
    }

    vll order;
    bfs(edges, start, order);
    if (order.size() != N) {
        print("-1");
    } else {
        repd(i, N) {
