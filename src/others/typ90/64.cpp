...
int main() {
    ...
    auto Q = in_ll();
    auto A = in_vll(N);
    auto LRV = in_vvll(3, Q);

    lazy_segtree<ll, op, e, ll, mapping, composition, id> tree(A);

    ll ans = 0;
    reps(i, 1, N) ans += abs(A[i] - A[i - 1]);

    repi(lrv, LRV) {
        ll l, r;
        if (lrv[0] != 1) {
            l = abs(tree.get(lrv[0] - 1) - tree.get(lrv[0] - 2));
        }
        if (lrv[1] != N) {
            r = abs(tree.get(lrv[1]) - tree.get(lrv[1] - 1));
        }
        tree.apply(lrv[0] - 1, lrv[1], lrv[2]);
        if (lrv[0] != 1) {
            l -= abs(tree.get(lrv[0] - 1) - tree.get(lrv[0] - 2));
            ans -= l;
        }
