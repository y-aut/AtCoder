...
inline vm concat(vm &a, vm &b) {
    return {a[0] * (b[0] + b[2]) + a[1] * b[0], a[0] * (b[1] + b[3]) + a[1] * b[1],
            a[2] * (b[0] + b[2]) + a[3] * b[0], a[2] * (b[1] + b[3]) + a[3] * b[1]};
}
...
int main() {
    ...
    // dp[i]: N = 2^i, (先頭, 末尾) が (0, 0), (0, 1), (1, 0), (1, 1)
    vector<vm> dp;
    dp.pb({1, 0, 0, 1});
    ll n = 1;

    while ((n <<= 1) <= N) {
        dp.pb(concat(dp[dp.size() - 1], dp[dp.size() - 1]));
    }

    vm ans = dp[dp.size() - 1];
    ll tmp = N ^ (n >> 1);
    for (ll i = 0, mask = 1; tmp; i++, mask <<= 1) {
        if (tmp & mask) {
            tmp ^= mask;
            ans = concat(ans, dp[i]);
        }
    ...
    print(accumulate(all(ans), (mint)0));
