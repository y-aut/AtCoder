...
int dfs(ll n, ll b, ll n_size, string str, ll nex, ll f) {
    ll cnt = 0;
    string bf;
    if (1 <= b + f && b + f <= n && str.size() == (bf = to_string(b + f)).size()) {
    ...
    for (ll i = nex; i <= 9; i++) {
        auto str2 = str; str2 += ('0' + i);
        cnt += dfs(n, b, n_size, str2, i, f * i);
    }
    return cnt;
}
...
int main()
{
    ...
    auto cnt = dfs(N, B, to_string(N).size(), "", 0, 1);
    print(cnt);
