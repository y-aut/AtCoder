...
void solve() {
    ...
    ll n = 1 << K;
    VLL(R, n);
    auto f = [&](auto rc, ll v) -> um<ll, double> {
        um<ll, double> ans;
        if (v >= n) {
            ans[v - n] = 1.;
            return ans;
        }
        auto left = rc(rc, v * 2), right = rc(rc, v * 2 + 1);
        repi(i, p, left) repi(j, q, right) {
            double lwin = 1. / (1 + pow(10., (R[j] - R[i]) / 400.));
            ans[i] += p * q * lwin;
            ans[j] += p * q * (1 - lwin);
        }
        return ans;
