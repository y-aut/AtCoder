...
void solve() {
    ...
    STR(S);
    vvb cand;
    repi(i, words) cand.pb(vb(i.size()));
    cand[0].back() = true;
    repi(c, S) {
        vvb nxt;
        repi(i, words) nxt.pb(vb(i.size()));
        bool end = false;
        repi(i, cand) if (i.back()) BREAK(end = true);
        if (end) {
            rep(i, words.size()) {
                if (c == words[i][0]) nxt[i][0] = true;
            }
        }
        rep(i, words.size()) rep(j, words[i].size() - 1) {
            if (cand[i][j] && c == words[i][j + 1]) nxt[i][j + 1] = true;
        }
        cand = move(nxt);
    }
    repi(i, cand) if (i.back()) EXIT(YES);
    return NO;
}
