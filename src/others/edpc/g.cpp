...
ll dfs(const vvll &edges, const ll v, const ll last, vll &dp) {
    if (dp[v]) return dp[v];
    ll ans = 0;
    repi(i, edges[v]) {
        if (i == last) continue;
        chmax(ans, dfs(edges, i, v, dp) + 1);
    }
    chmax(dp[v], ans);
    return dp[v];
}
...
int main() {
    ...
    usll in_0;
    rep(i, N) in_0.insert(i);
    repi(i, edges) repi(j, i) in_0.erase(j);

    ll ans = 0;
    repi(i, in_0) {
        vll dp(N, 0);
        chmax(ans, dfs(edges, i, -1, dp));
    }
