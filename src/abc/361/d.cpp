...
void solve() {
    ...
    STR(T);
    um<string, ll> ls;
    auto f = [&](auto rc, string &cur, ll d, ll dmax, ll b, ll last) -> void {
        if (ls.count(cur) && ls[cur] <= d) return;
        ls[cur] = d;
        if (d == dmax) return;
        rep(i, cur.size() - 1) {
    ...
    debug(ls);
    ll ans = LINF;
    auto g = [&](auto rc, string &cur, ll d, ll dmax, ll b, ll last) -> void {
        if (ls.count(cur)) {
            chmin(ans, d + ls[cur]);
            return;
        }
