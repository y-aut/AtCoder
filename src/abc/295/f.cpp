...
// 1 ~ limit
ll solve_max(const string &S, ll limit) {
    if (limit == 0) return 0;
    auto lim_s = to_string(limit);
    int digit = lim_s.size();
    if (digit < S.size()) return 0;
    ll ans = 0;
    auto S_ll = stoll(S);
    rep(i, digit - S.size() + 1) {
        auto sub_ll = stoll(lim_s.substr(i, S.size()));
        if (i == 0) {
            if (S[0] == '0') continue;
            if (S_ll == sub_ll) {
                if (S.size() == digit) ans++;
                else ans += stoll(lim_s.substr(S.size())) + 1;
            } else if (S_ll < sub_ll) {
                ans += pow10[digit - S.size()];
            }
        } else {
            ll head = stoll(lim_s.substr(0, i));
            if (S[0] != '0') head++;
            if (S_ll >= sub_ll) head--;
            ans += head * pow10[digit - S.size() - i];
            if (S_ll == sub_ll) {
                if (i + S.size() == digit) ans++;
                else ans += stoll(lim_s.substr(i + S.size())) + 1;
            }
        }
    }
    return ans;
...
int main() {
    pow10[0] = 1;
    reps(i, 1, 18) pow10[i] = pow10[i - 1] * 10;

    INLL(T);
    rep(t, T) {
        auto S = in_str();
        INLL(L, R);
        print(solve_max(S, R) - solve_max(S, L - 1));
    }
