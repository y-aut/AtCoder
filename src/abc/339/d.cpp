...
inline ll zip(ll a, ll b, ll max_b) {
    return a * max_b + b;
}

inline ll zip(pll p, ll max_b) {
    return p.first * max_b + p.second;
}

inline pll unzip(ll i, ll max_b) {
    return {i / max_b, i % max_b};
}

pll moved(ll N, const vs &S, const pll &p, ll side) {
    pll ans{p.first + DY[side], p.second + DX[side]};
    if (!(0 <= ans.first && ans.first < N && 0 <= ans.second && ans.second < N)) {
        return p;
    }
    if (S[ans.first][ans.second] == '#') {
        return p;
    }
    return ans;
}

vll next_s(ll N, const vs &S, ll p) {
    auto tmp = unzip(p, N * N);
    auto p1 = unzip(tmp.first, N);
    auto p2 = unzip(tmp.second, N);
    vll ans;
    rep(i, 4) {
        auto new_p1 = moved(N, S, p1, i);
        auto new_p2 = moved(N, S, p2, i);
        ans.pb(zip(zip(new_p1, N), zip(new_p2, N), N * N));
    }
    return ans;
}
...
void solve() {
    ...
    rep(i, N) rep(j, N) {
        if (S[i][j] == 'P') {
            if (p1 == -1) p1 = zip(i, j, N);
            else BREAK(p2 = zip(i, j, N));
        }
    }

    ll cur = zip(p1, p2, N * N);
    umll ans;
    ans[cur] = 0;

    queue<ll> q;
    // {
    //     auto nx = next_s(N, S, cur);
    //     repi(i, nx) {
