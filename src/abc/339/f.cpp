...
void solve() {
    ...
    vector<modint1000000007> M1;
    vector<modint998244353> M2;
    repi(i, A) {
        M1.pb(((mll)(i % (mll)((int)MOD))).get_ui());
        M2.pb(((mll)(i % (mll)((int)MOD2))).get_ui());
    }
    usll M1_s, M2_s;
    repi(i, M1) M1_s.insert(i.val());
    repi(i, M2) M2_s.insert(i.val());

    ll ans = 0;
    rep(i, N) rep(j, i, N) {
        auto m1 = M1[i] * M1[j];
        if (!M1_s.count(m1.val())) continue;
        auto m2 = M2[i] * M2[j];
        if (!M2_s.count(m2.val())) continue;
        auto p = A[i] * A[j];
        auto lptr = lower_bound(all(A), p);
        if (lptr == A.end()) continue;
        auto rptr = upper_bound(all(A), p);
        ans += distance(lptr, rptr) * (i == j ? 1 : 2);
    }
    print(ans);
}
