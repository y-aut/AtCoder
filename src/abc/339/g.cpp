...
ll op1(ll a, ll b) { return min(0LL, a) + min(0LL, b); }
ll op2(ll a, ll b) {
    if (a < MOD) a = a <= 0 ? MOD + 1 : MOD;
    if (b < MOD) b = b <= 0 ? MOD + 1 : MOD;
    return a + b - MOD;
}
ll e1() { return 0; }
ll e2() { return MOD; }
...
void solve() {
    ...
    lazy_segtree<ll, op1, e1, ll, mapping, composition, id> tree1(N);
    lazy_segtree<ll, op1, e2, ll, mapping, composition, id> tree2(N);
    rep(i, N) tree1.set(i, A[i]);
    rep(i, N) tree2.set(i, A[i]);
    ...
    rep(q, Q) {
        ...
        r ^= ans;
        x ^= ans;
        tree1.apply(l, r, x - lastx);
        tree2.apply(l, r, x - lastx);
        ans = tree1.prod(l, r) + x * (tree2.prod(l, r) - MOD);
        lastx = x;
        print(ans);
    }
}
