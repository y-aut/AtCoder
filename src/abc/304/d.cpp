...
inline ll index(ll x, ll y, ll A) {
    return x + y * (A + 1);
}
...
int main() {
    ...
    repi(i, berry) {
        auto x = lower_bound(all(a), i.first) - a.begin();
        auto y = lower_bound(all(b), i.second) - b.begin();
        cnt[index(x, y, A)]++;
    }
    ...
    repi(i, cnt) {
        chmin(ans.first, i.second);
        chmax(ans.second, i.second);
    }
