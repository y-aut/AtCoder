...
void solve() {
    ...
    rep(i, H) rep(j, W) {
        ...
        if (A[i][j] == 'G') G = {i, j}, A[i][j] = '.';
        ll di = 0, dj = 0;
        if (A[i][j] == '>') dj = 1;
        else if (A[i][j] == 'v') di = 1;
        else if (A[i][j] == '<') dj = -1;
        else if (A[i][j] == '^') di = -1;
        if (di == 0 && dj == 0) continue;
        A[i][j] = '#';
        for (ll ni = i + di, nj = j + dj; BTW(ni, 0, H, nj, 0, W) && (A[ni][nj] == '.' || A[ni][nj] == 'p'); ni += di, nj += dj) {
            A[ni][nj] = 'p';
        }
    }
    queue<pll> q;
    ...
    while (!q.empty()) {
        ...
        rep(d, 4) {
            ...
            if (!BTW(ni, 0, H, nj, 0, W)) continue;
            if (A[ni][nj] != '.') continue;
            if (dist[ni][nj] != LINF) continue;
            dist[ni][nj] = dist[i][j] + 1;
            q.emplace(ni, nj);
        }
    }
    printex(dist[G.first][G.second], LINF, -1);
}
