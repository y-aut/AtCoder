...
void solve() {
    ...
    repi(i, a) i--;
    usll v, u;
    rep(i, N) v.insert(i), u.insert(i);
    repi(i, a) v.erase(i);
    ll ans = 0;
    vector<usll *> loops(N, nullptr);
    vll cnt(N);
    repi(st, v) {
        auto ex = new usll();
        vll vs;
        ...
        do {
            ...
            u.erase(cur);
            cur = a[cur];
        } while (!ex->count(cur) && loops[cur] == nullptr && cnt[cur] == 0);
        if (loops[cur] == nullptr) {
            rep(i, vs.size()) {
                if (vs[i] == cur) {
                    rep(j, i, vs.size()) {
                        loops[vs[j]] = ex;
                ...
                ex->erase(vs[i]);
                ans += vs.size() - i + cnt[cur];
                cnt[vs[i]] = vs.size() - i + cnt[cur];
            }
        } else {
            rep(i, vs.size()) {
                if (vs[i] == cur) break;
                ans += vs.size() + loops[cur]->size() - i;
                cnt[vs[i]] = vs.size() + loops[cur]->size() - i;
            }
        }
    }
    while (!u.empty()) {
