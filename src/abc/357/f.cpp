...
dat e() { return {0, 0, 0, 0}; }
dat mapping(pll f, dat x) {
    x.absum += f.first * x.bsum;
    x.asum += f.first * x.size;
    x.absum += f.second * x.asum;
    x.bsum += f.second * x.size;
    return x;
}
pll composition(pll f, pll g) {
    f.first += g.first;
    f.second += g.second;
    return f;
}
...
void solve() {
    ...
    vector<dat> data;
    rep(i, N) {
        data.pb({A[i], B[i], A[i] * B[i], 1});
    }
    lazy_segtree<dat, op, e, pll, mapping, composition, id> tree(data);
    rep(q, Q) {
        LL(op, l, r);
        l--;
        if (op == 1) {
