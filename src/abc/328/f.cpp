...
public:
    UnionFind(ll _size) : size(_size), parents(size, -1), difs(size) {}
    ...
    void merge(ll v1, ll v2, ll dif) {
        auto r = root(v2);
        difs[r] = -(dif + height(v2));
        parents[r] = v1;
    }
};
...
void solve() {
    ...
    vll ans;
    rep(i, Q) {
        auto v1 = abd[i][0], v2 = abd[i][1], d = abd[i][2];
        if (uf.is_connected(v1, v2)) {
            if (uf.height(v1) == uf.height(v2) + d) {
                ans.pb(i + 1);
            }
        } else {
            uf.merge(v1, v2, d);
