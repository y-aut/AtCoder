...
ll dfs(ll N, ll M, ll K, vvll edges, ll ind, ll cnt, ll sum, dsu &uf) {
    if (cnt == N - 1) {
        if (uf.groups().size() == 1) return sum % K;
        return LINF;
    }
    if (ind == M) return LINF;
    ll ans = LINF;
    chmin(ans, dfs(N, M, K, edges, ind + 1, cnt, sum, uf));
    auto uf2 = uf;
    uf2.merge(edges[ind][0], edges[ind][1]);
    chmin(ans, dfs(N, M, K, edges, ind + 1, cnt + 1, sum + edges[ind][2], uf2));
    return ans;
}
...
void solve() {
    ...
    dsu uf(N);
    print(dfs(N, M, K, edges, 0, 0, 0, uf));
}
