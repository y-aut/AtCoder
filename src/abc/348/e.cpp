...
void set_partial_c(const Tree &tree, const vll &C, vll &partial_c, ll v) {
    ll ans = C[v];
    repi(i, tree.get_children(v)) {
        set_partial_c(tree, C, partial_c, i);
        ans += partial_c[i];
    }
    partial_c[v] = ans;
}

ll get_min(const Tree &tree, const vll &partial_c, ll node, ll val) {
    ll ans = val;
    repi(i, tree.get_children(node)) {
        ll new_val = val - partial_c[i] + (partial_c[0] - partial_c[i]);
        chmin(ans, get_min(tree, partial_c, i, new_val));
    }
    return ans;
}
...
void solve() {
    ...
    vvll edges(N);
    repi(i, AB) {
        edges[i.first - 1].pb(i.second - 1);
        edges[i.second - 1].pb(i.first - 1);
    }

    Tree tree(edges);
    vll partial_c(N);
    set_partial_c(tree, C, partial_c, 0);

    ll root = 0;
    rep(i, 1, N) {
        root += C[i] * tree.get_depth(i);
    }
    print(get_min(tree, partial_c, 0, root));
}
