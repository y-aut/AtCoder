...
void solve() {
    ...
    pll start;
    rep(i, H) rep(j, W) {
        if (A[i][j] == 'S') BREAK(start = pll(i, j));
    }
    ...
    while (!q.empty()) {
        auto cur = q.front();
        q.pop();
        chmax(e[cur.first][cur.second], M[cur.first][cur.second]);
        if (e[cur.first][cur.second] == 0) continue;
        rep(i, 4) {
            pll nxt{cur.first + DY[i], cur.second + DX[i]};
            if (!(0 <= nxt.first && nxt.first < H && 0 <= nxt.second && nxt.second < W)) continue;
            if (A[nxt.first][nxt.second] == '#') continue;
            if (A[nxt.first][nxt.second] == 'T') EXIT(Yes);
            if (chmax(e[nxt.first][nxt.second], e[cur.first][cur.second] - 1)) {
                q.push(nxt);
            }
        }
    }
