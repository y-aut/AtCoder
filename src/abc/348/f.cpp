...
void solve() {
    ...
    vector<vector<ull>> s(1000, vector<ull>((N + 63) / 64));
    vector<vector<ull>> bit(N, vector<ull>((N + 63) / 64));
    rep(j, M) {
        rep(i, N) {
            s[A[i][j]][mask[i].first] |= mask[i].second;
        }
        rep(i, N) {
            rep(k, bit[i].size()) {
                bit[i][k] ^= s[A[i][j]][k];
        ...
        repi(i, s) rep(k, i.size()) i[k] = 0;
    }

    ll ans = 0;
    repi(i, bit) repi(j, i) ans += pcntll(j);
    if (M % 2 == 1) ans -= N;
    print(ans / 2);
}
