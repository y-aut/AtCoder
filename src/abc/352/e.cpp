...
void solve() {
    ...
    dsu uf(N);
    sort(all(Q), [](const pair<pll, vll> &a, const pair<pll, vll> &b) {
        return a.first.second < b.first.second;
    });

    ll ans = 0;
    repi(q, Q) {
        rep(i, q.second.size() - 1) {
            if (!uf.same(q.second[i], q.second[i + 1])) {
                uf.merge(q.second[i], q.second[i + 1]);
                ans += q.first.second;
            }
        }
    }
