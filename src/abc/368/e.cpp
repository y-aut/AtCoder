...
void solve() {
    ...
    rep(i, M) {
        info[i][0]--, info[i][1]--;
        ids[info[i][0]][info[i][2]].pb(i);
    }
    auto dfs = [&](auto rc, ll v, ll x) -> void {
        auto &[a, b, s, t] = info[v];
        auto itr = ids[b].lower_bound(t);
        s += x;
        t += x;
        while (itr != ids[b].end() && itr->first < t) {
            repi(i, itr->second) {
                rc(rc, i, t - itr->first);
            }
            itr++;
    ...
    dfs(dfs, 0, X);
    vll ans(M - 1);
    rep(i, 1, M) ans[i - 1] = info[i][2] - def[i][2];
    print(ans);
}
