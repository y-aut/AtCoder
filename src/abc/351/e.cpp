...
ll f(const vpll &p) {
    if (p.empty()) return 0;
    vll cx, cy;
    if ((p[0].first + p[0].second) % 2 == 1) {
        repi(i, p) {
            cx.pb((i.first + i.second + 1) / 2);
            cy.pb((i.second - i.first + 1) / 2);
        }
    } else {
        repi(i, p) {
            cx.pb((i.first + i.second) / 2);
            cy.pb((i.second - i.first) / 2);
        }
    }
    return g(cx) + g(cy);
}
...
void solve() {
    ...
    vpll os, es;
    repi(i, P) {
        if ((i.first + i.second) % 2 == 1) os.pb(i);
        else es.pb(i);
    }
    print(f(os) + f(es));
}
