...
int main() {
    ...
    usll rrest, crest;
    rep(i, H) rrest.insert(i);
    rep(i, W) crest.insert(i);

    vvll rcnt(H, vll(26, 0)), ccnt(W, vll(26, 0));
    rep(i, H) rep(j, W) {
        rcnt[i][c[i][j] - 'a']++;
        ccnt[j][c[i][j] - 'a']++;
    }
    ...
    while (true) {
        vpll to_delete_r, to_delete_c;

        if (crest.size() > 1) {
            repi(r, rrest) {
                rep(a, 26) {
                    if (rcnt[r][a] == 0) continue;
                    if (rcnt[r][a] != crest.size()) break;
                    to_delete_r.eb(r, a);
                    break;
                }
            }
        ...
        if (rrest.size() > 1) {
            repi(c, crest) {
                rep(a, 26) {
                    if (ccnt[c][a] == 0) continue;
                    if (ccnt[c][a] != rrest.size()) break;
                    to_delete_c.eb(c, a);
                    break;
                }
            }
        ...
        repi(p, to_delete_r) {
            rrest.erase(p.first);
            repi(c, crest) ccnt[c][p.second]--;
        }
        repi(p, to_delete_c) {
            crest.erase(p.first);
            repi(r, rrest) rcnt[r][p.second]--;
        }

        if (to_delete_r.empty() && to_delete_c.empty()) break;
    }

    print(rrest.size() * crest.size());
