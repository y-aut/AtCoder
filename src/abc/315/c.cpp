...
int main() {
    ...
    vector<pll> dict(N, pll(0, 0));
    repi(p, FS) {
        if (dict[p.first].first == 0) {
            dict[p.first].first = p.second;
        } else if (dict[p.first].second == 0) {
            dict[p.first].second = min(dict[p.first].first, p.second);
            dict[p.first].first = max(dict[p.first].first, p.second);
        } else {
            if (p.second >= dict[p.first].first) {
                dict[p.first].second = dict[p.first].first;
                dict[p.first].first = p.second;
            } else if (p.second >= dict[p.first].second) {
                dict[p.first].second = p.second;
            }
        }
    }

    vll first;
    rep(i, N) first.pb(dict[i].first);
    sort(all(first), greater<ll>());

    ll ans = first[0] + first[1];
    rep(i, N) {
        if (dict[i].second != 0) {
            chmax(ans, dict[i].first + dict[i].second / 2);
        }
    }
    print(ans);
