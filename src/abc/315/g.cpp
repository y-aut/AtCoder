...
inline ll flr(ll a, ll b) {
    if (a % b != 0 && a < 0) return a / b - 1;
    else return a / b;
}

int main() {
    LL(N, A, B, C, X);
    auto mx = max(A, max(B, C));
    auto mn = min(A, min(B, C));
    auto md = A + B + C - mx - mn;
    ...
    rrep(k, N) {
        ...
        vll a{B, A}, b;
        while (a.back() != 1) {
            b.pb(a[a.size() - 2] / a.back());
            a.pb(a[a.size() - 2] % a.back());
        }
        ll p = 1, q = -b.back();
        b.pop_back();
        while (!b.empty()) {
        ...
        ll kmin = max(flr(p * Y - N - 1, A) + 1, flr(1 - q * Y - 1, B) + 1);
        ll kmax = min(flr(p * Y - 1, A), flr(N - q * Y, B));
        if (kmin > kmax) continue;
        ans += kmax - kmin + 1;
    }
