...
void solve() {
    ...
    repd(i, M) {
        if (TAX[i][0] == 1) {
            if (r.count(TAX[i][1])) continue;
            r.insert(TAX[i][1]);
            if (W - c.size() > 0) ans[TAX[i][2]] += W - c.size();
        } else {
            if (c.count(TAX[i][1])) continue;
            c.insert(TAX[i][1]);
            if (H - r.size() > 0) ans[TAX[i][2]] += H - r.size();
        }
    }
    ll rest = (H - r.size()) * (W - c.size());
    if (rest > 0) ans[0] += rest;
    print(ans.size());
    print(ans);
}
