...
bool ok(ll N, const string &S, const string &T, um<char, vll> &pos, ll k) {
    if (k > S.size() * N / T.size() + 2) return false;
    ll cur = 0;
    rep(i, T.size()) {
        if ((S.size() * N - cur) / k + 2 < T.size() - i) return false;
        auto &&buf = pos[T[i]];
        if (buf.empty()) return false;
        ll ind = lower_bound(all(buf), cur % S.size()) - buf.begin();
        if (buf.size() - ind >= k) {
            cur = cur / S.size() * S.size() + buf[ind + k - 1] + 1;
        } else {
            cur = (cur / S.size() + 1) * S.size();
            ll k_rest = k - (buf.size() - ind) - 1;
            cur += S.size() * (k_rest / buf.size()) + buf[k_rest % buf.size()] + 1;
        }
        if (cur > S.size() * N) return false;
    }
    return true;
}
...
void solve() {
    ...
    um<char, vll> pos;
    rep(i, S.size()) pos[S[i]].pb(i);

    ll lower = 0, upper = LINF;
    while (lower + 1 < upper) {
        ll middle = (lower + upper) / 2;
        if (ok(N, S, T, pos, middle)) {
            lower = middle;
        } else {
            upper = middle;
        }
