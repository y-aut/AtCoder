...
void solve() {
    ...
    if ((S.size() % 2 == 0) ^ (S[S.size() - 1] != '0')) {
        tl0.pb(0);
        tl1.pb(C[S.size() - 1]);
        tl_last = 1 - (S[S.size() - 1] - '0');
    } else {
        tl0.pb(C[S.size() - 1]);
        tl1.pb(0);
        tl_last = S[S.size() - 1] - '0';
    }
    ll nxt = 1;
    rep(i, 1, N) {
        if (S[i] - '0' == nxt) {
            hd0.pb(hd0[hd0.size() - 1] + 0);
            hd1.pb(hd1[hd1.size() - 1] + C[i]);
        } else {
            hd0.pb(hd0[hd0.size() - 1] + C[i]);
            hd1.pb(hd1[hd1.size() - 1] + 0);
        }
        nxt = 1 - nxt;
    }
    nxt = tl_last;
    repd(i, N - 1) {
        if (S[i] - '0' == nxt) {
            tl0.pb(tl0[tl0.size() - 1] + 0);
            tl1.pb(tl1[tl1.size() - 1] + C[i]);
        } else {
            tl0.pb(tl0[tl0.size() - 1] + C[i]);
            tl1.pb(tl1[tl1.size() - 1] + 0);
        }
        nxt = 1 - nxt;
    }
    ll ans = LINF;
    rep(i, N - 1) {
        chmin(ans, hd0[i] + tl0[N - 2 - i]);
        chmin(ans, hd1[i] + tl1[N - 2 - i]);
    }
    print(ans);
}
