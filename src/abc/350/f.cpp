...
pll inv(const pll &a) {
    return {1 / a.first, -a.second / a.first};
}
...
void solve() {
    ...
    um<ll, pll> cor; // pos, char of start, char size
    stack<pll> s;
    ll ci = 0;
    rep(i, S.size()) {
        if (S[i] == '(') s.emplace(i, ci);
        else if (S[i] == ')') {
            cor[s.top().first] = {s.top().second, ci - s.top().second};
            s.pop();
        } else {
            ci++;
        }
    ...
    rep(i, S.size()) {
        if (S[i] == '(') {
            pll f{-1, cor[i].first * 2 + cor[i].second - 1};
            sf.push(cmp(f, sf.top()));
        } else if (S[i] == ')') {
            sf.pop();
        } else {
            ans[app(sf.top(), ci)] = sf.size() % 2 == 1 ? S[i] : sw(S[i]);
            ci++;
        }
    }
    print(ans);
