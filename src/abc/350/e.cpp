...
void solve() {
    ...
    um<ll, double> memo;
    auto f = [&](auto rec, ll v) -> double {
        if (memo.count(v)) return memo[v];
        if (v == 0) return memo[v] = 0;
        return memo[v] = min(rec(rec, v / A) + X,
                             (rec(rec, v / 2) + rec(rec, v / 3) + rec(rec, v / 4) + rec(rec, v / 5) + rec(rec, v / 6)) / 5 + (double)Y * 6 / 5);
    };
