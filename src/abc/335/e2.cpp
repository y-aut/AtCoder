...
void solve() {
    ...
    rep(i, M) {
        LL(u, v);
        u--, v--;
        if (A[u] <= A[v]) edges[u].pb(v);
        if (A[v] <= A[u]) edges[v].pb(u);
    }
    vll score(N);
    score[0] = 1;
    priority_queue<pair<pll, ll>, v<pair<pll, ll>>, greater<pair<pll, ll>>> q;
    q.emplace(pll{A[0], -score[0]}, 0);
    while (!q.empty()) {
        auto [pr, v] = q.top();
        q.pop();
        if (score[v] > -pr.second) continue;
        repi(i, edges[v]) {
            ll sc = score[v] + (A[i] > A[v]);
            if (sc > score[i]) {
                score[i] = sc;
                q.emplace(pll{A[i], -score[i]}, i);
            }
        }
    }
    print(score[N - 1]);
