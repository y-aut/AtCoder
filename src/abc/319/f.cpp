...
private:
    ...
    void get_subtree_impl(ll v, usll &res) const {
        res.insert(v);
        repi(i, get_children(v)) get_subtree_impl(i, res);
    }
...
public:
    ...
    usll get_subtree(ll v) const {
        usll ans;
        get_subtree_impl(v, ans);
        return ans;
    }
...
void solve() {
    ...
    rep(i, 1, N) {
        ...
        edges[i].pb(p);
        edges[p].pb(i);
        if (t == 1) ene_ind[i] = ene.size(), ene.pb({i, s, g}), chmax(max_str, s);
        else med_ind[i] = med.size(), med.pb({i, g});
    }
    Tree tree(edges);
