...
void add(vvll &ptsg, vvll &children, set<pll> &ene, set<pll> &medi, ll v) {
    repi(c, children[v]) {
        if (ptsg[c][1] == 1) ene.emplace(ptsg[c][2], c);
        else medi.emplace(ptsg[c][3], c);
    }
}

ll exec(vvll &ptsg, vvll &children, set<pll> &ene, set<pll> &medi, ll stren) {
    while (!ene.empty()) {
        if (stren >= ene.begin()->first) {
            stren += ptsg[ene.begin()->second][3];
            add(ptsg, children, ene, medi, ene.begin()->second);
            ene.erase(ene.begin());
            continue;
        } else break;
    }
...
void calc(vector<tuple<ll, set<pll>, set<pll>>> &dp, vvll &ptsg, vvll &children, vll &medis, ll i) {
    if (get<0>(dp[i])) return;
    for (ll j = 0, mask = 1; j < medis.size(); j++, mask <<= 1) {
        if (i & mask) {
            calc(dp, ptsg, children, medis, i ^ mask);
            auto stren = get<0>(dp[i ^ mask]);
            auto ene = get<1>(dp[i ^ mask]);
            auto medi = get<2>(dp[i ^ mask]);
            bool flg = false;
            repi(m, medi) {
                if (m.second == medis[j]) {
                    flg = true;
            ...
            if (!flg) continue;
            stren *= ptsg[medis[j]][3];
            if (stren >= 1000000000) EXIT(Yes);
                }
    }
}
...
int main() {
    ...
    vvll children(N, vll());
    rep(i, N - 1) {
        children[ptsg[i][0] - 1].pb(i + 1);
    }

    vll medis;
    rep(i, N - 1) if (ptsg[i][1] == 2) medis.pb(i + 1);

    ptsg.insert(ptsg.begin(), vll());

    vector<tuple<ll, set<pll>, set<pll>>> dp(1LL << medis.size(), {0, set<pll>(), set<pll>()});
    set<pll> ene, medi;
    ll stren = exec(ptsg, children, ene, medi, 1);
    dp[0] = {stren, ene, medi};

    ll area_cnt = 1;
    set<pll> ene, medi;
    add(ptsg, children, ene, medi, 0);

    ll stren = 1;
    while (area_cnt < N) {
        bool flg = false;
        if (stren >= 1000000000LL) EXIT(Yes);
        while (!ene.empty()) {
            if (ene.begin()->first <= stren) {
                stren += ptsg[ene.begin()->second][3];
                add(ptsg, children, ene, medi, ene.begin()->second);
                area_cnt++;
                ene.erase(ene.begin());
        ...
        if (medi.empty()) break;
        while (!medi.empty()) {
            stren *= ptsg[medi.begin()->second][3];
            if (stren >= 1000000000LL) EXIT(Yes);
            add(ptsg, children, ene, medi, medi.begin()->second);
            area_cnt++;
            medi.erase(medi.begin());
        }
    }
