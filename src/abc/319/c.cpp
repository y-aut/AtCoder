...
bool is_gakkari(vvll &cur) {
    rep(i, 3) {
        ...
        rep(j, 3) {
            if (cur[i][j]) {
                if (n1 == 0) n1 = cur[i][j];
                else if (n2 == 0) n2 = cur[i][j];
                else BREAK(flg = true);
            }
        }
        if (!flg && n2 && n1 == n2) return true;
    }
    rep(i, 3) {
        ll n1 = 0, n2 = 0;
        bool flg = false;
    ...
    }
    {
        ...
        rep(j, 3) {
            if (cur[j][2 - j]) {
                if (n1 == 0) n1 = cur[j][2 - j];
                else if (n2 == 0) n2 = cur[j][2 - j];
                else BREAK(flg = true);
            }
        }
        if (!flg && n2 && n1 == n2) return true;
...
bool is_gakkari(vvll &c, vll &order) {
    vvll cur(3, vll(3, 0));
    repi(i, order) {
        cur[i / 3][i % 3] = c[i / 3][i % 3];
        if (is_gakkari(cur)) return true;
    }
    return false;
}
...
int main() {
    ...
    do {
        sum++;
        if (is_gakkari(c, order)) cnt++;
    } while (next_permutation(all(order)));

    cout << setprecision(10);
    print((double)(sum - cnt) / sum);
