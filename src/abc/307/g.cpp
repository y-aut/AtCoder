...
int main() {
    ...
    vll sum{A[0]};
    reps(i, 1, N) sum.pb(sum.back() + A[i]);

    auto m = sum.back() / N;
    if (sum.back() < 0 && sum.back() % N) m--;
    auto over = sum.back() - m * N;
    ...
    rep(i, N) {
        ...
        vll tmp(N, LINF);
        rep(j, over + 1) {
            auto val = dp[j] + abs(sum[i] - A[i] - (m * i + j));
            if (j) chmin(val, dp[j - 1] + abs(sum[i] - A[i] - (m * i + j - 1)));
            if (val <= LINF) tmp[j] = val;
        }
        dp = move(tmp);
    }
