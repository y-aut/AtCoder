...
class ReRooting : Tree {
    ...
    const function<T(T, ll)> mapping_up;

public:
    // 単方向の辺を受け取る
    ReRooting(const vvll &_edges, function<T()> _e, function<T(T, T)> _op, function<T(T, ll)> _mapping, function<T(T, ll)> _mapping_up)
        : Tree(_edges, 0), index(size, vll()), res(size), e(_e), op(_op), mapping(_mapping), mapping_up(_mapping_up) {
        ...
        rep(i, order.size()) {
            ...
            rep(j, accums_rev.size()) {
                dp[edges[node][j]][index[node][j]] =
                    mapping_up(op(accum, accums_rev[j]), node);
                accum = op(accum, dp[node][j]);
            }
            res[node] = mapping_up(accum, node);
        }
    }
};
...
ll mp(ll f, ll x) { return f + cnt[x]; }
ll mp2(ll f, ll x) { return f + x - cnt[x]; }
...
void solve() {
    ...
    vvll memo(N, vll((N + width - 1) / width));
    auto f = [&](auto rc, ll v) -> set<ll> {
        set<ll> ans{v};
        repi(i, tree.get_children(v)) {
            auto cld = rc(rc, i);
            if (cld.size() > ans.size()) swap(cld, ans);
            repi(j, cld) ans.insert(j);
            rep(j, memo[0].size()) memo[v][j] += memo[i][j];
        }
        for (ll i = 0, j = 0; i < N; i += width, j++) {
            memo[v][j] += v < i;
        }
        ll c = memo[v][v / width];
        auto itr = ans.lower_bound((v / width) * width);
        while (itr != ans.end() && *itr < v) itr++, c++;
        cnt[v] = c;
        return ans;
    };
    f(f, tree.get_root());
    ...
    ReRooting<ll> rr(edges, e, op, mp, mp2);
    vll ans;
    rep(i, N) ans.pb(rr.get(i));
    print(ans);
}
