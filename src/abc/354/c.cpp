...
void solve() {
    ...
    set<ll> as;
    repi(i, AC) as.insert(i.first);
    vll as_v;
    repi(i, as) as_v.pb(i);
    umll a_map;
    rep(i, as_v.size()) a_map[as_v[i]] = i;

    repi(i, AC) i.first = a_map[i.first];
    segtree<ll, op, e> tree(as.size());

    umll ind;
    rep(i, AC.size()) {
        ind[AC[i].first * INF + AC[i].second] = i + 1;
    }

    sort(all(AC), [](const pll &a, const pll &b) {
        return a.first - a.second > b.first - b.second;
    });

    vll ans;
    repi(i, AC) {
        if (tree.prod(i.first + 1, as.size()) < i.second) continue;
        else {
            ans.pb(ind[i.first * INF + i.second]);
            tree.set(i.first, i.second);
        }
    }
    sort(all(ans));
    print(ans.size());
