...
void solve() {
    ...
    vll dp(T.size() + 1, LINF);
    dp[0] = 0;
    rep(i, N) {
        repd(j, dp.size()) if (dp[j] != LINF) {
            repi(s, S[i]) if (j + s.size() <= T.size() && s == T.substr(j, s.size())) {
                chmin(dp[j + s.size()], dp[j] + 1);
            }
        }
    }
    print(dp[T.size()] == LINF ? -1 : dp[T.size()]);
}
