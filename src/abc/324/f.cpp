...
bool check(ll N, vector<vvll> &edges, double v) {
    vd dp(N, -LINF);
    dp[0] = 0;
    rep(i, 1, N) {
        repi(j, edges[i]) {
            chmax(dp[i], dp[j[0]] + j[2] - v * j[3]);
        }
    }
    return dp[N - 1] >= 0;
}
...
int main() {
    ...
    vector<vvll> edges(N);
    rep(i, M) edges[ary[i][1]].pb(ary[i]);

    double upper = LINF, lower = 0;
    while (true) {
        if (upper - lower < 1e-11) break;
