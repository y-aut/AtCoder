...
void solve() {
    ...
    for (ll i = 0; i <= N; i += B) {
        vi less(N + 1);
        rep(j, N) less[j + 1] = less[j] + (A[j] <= i);
        acc_less.pb(less);
    }

    auto cnt_less = [&](ll l, ll r, ll x) -> ll {
        if (l >= r) return 0;
        if (x <= 0) return 0;
        if (x >= N) return r - l;
        ll ans = acc_less[x / B][r] - acc_less[x / B][l];
        rep(i, x / B * B + 1, x + 1) {
            ans += l <= inv[i] && inv[i] < r;
        }
        return ans;
    ...
    auto cnt_more_less = [&](ll l, ll r, ll lower, ll upper) -> ll {
        ...
        return max(0LL, cnt_less(l, r, upper - 1) - cnt_less(l, r, lower - 1));
    };
    ...
    rep(q, Q) {
        INT(t, s, x);
        if (t == 1) {
            int lower = arr[s][0], upper = arr[s][1] + 1;
            while (lower + 1 < upper) {
                int mid = (lower + upper) / 2;
                int c = cnt_more_less(arr[s][0], mid, arr[s][2], arr[s][3]);
                if (c == x) BREAK(lower = mid);
                if (c < x) lower = mid;
                else upper = mid;
            }
            arr.pb({lower, arr[s][1], arr[s][2], arr[s][3]});
            arr[s][1] = lower;
        } else {
