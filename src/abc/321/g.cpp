...
void solve() {
    ...
    repi(i, R) rcnt[i]++;
    repi(i, B) bcnt[i]++;
    vll rcntb(1LL << N), bcntb(1LL << N);
    rep(i, 1LL << N) {
        rep(j, N) if (i >> j & 1) rcntb[i] += rcnt[j], bcntb[i] += bcnt[j];
    }
    vm fact(M + 1);
    fact[0] = 1;
    rep(i, 1, M + 1) fact[i] = fact[i - 1] * i;
    vm one(1LL << N);
    rep(i, 1, 1LL << N) {
        if (rcntb[i] != bcntb[i]) continue;
        one[i] = fact[rcntb[i]];
        ll mask = 1;
        for (;; mask <<= 1)
            if (i & mask) break;
        ll rest = i ^ mask;
        if (rest == 0) continue;
        for (ll j = (rest - 1) & rest;; j = (j - 1) & rest) {
            ll group = j ^ mask;
            one[i] -= one[group] * fact[rcntb[i ^ group]];
            if (j == 0) break;
        }
    }
    // v<pmm> dp(1LL << N);
    ...
    //         if (i & mask) break;
    //     ll rest = i ^ mask;
    //     for (ll j = rest;; j = (j - 1) & rest) {
    //         ll group = j ^ mask;
    //         dp[i].first += dp[i ^ group].first * one[group];
    //         dp[i].second += (dp[i ^ group].first + dp[i ^ group].second) * one[group];
    //         if (j == 0) break;
    //     }
    // }
    // print(dp.back().second / fact[M]);
    mint ans = 0;
    rep(i, 1, 1LL << N) {
        ans += one[i] * fact[rcntb[((1LL << N) - 1) ^ i]];
    }
    print(ans / fact[M]);
}
