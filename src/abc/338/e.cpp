...
void solve() {
    ...
    repi(i, AB) {
        i.first--;
        i.second--;
        ll m = min(i.first, i.second);
        i.second = max(i.first, i.second);
        i.first = m;
    }
    // sort(all(AB), [](const pll &a, const pll &b) { return a.second < b.second; });

    vll gen(N * 2);
    rep(i, N) gen[AB[i].first] = gen[AB[i].second] = i;

    fenwick_tree<ll> tree(N * 2);
    rep(i, N * 2) {
        if (AB[gen[i]].first == i) tree.add(AB[gen[i]].first, 1);
        else {
            tree.add(AB[gen[i]].first, -1);
            if (tree.sum(AB[gen[i]].first + 1, AB[gen[i]].second) > 0) EXIT(Yes);
        }
    }
    No;
}
