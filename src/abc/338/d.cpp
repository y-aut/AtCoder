...
void solve() {
    ...
    lazy_segtree<ll, op, e, ll, add, add, e> tree(N);
    ll sum = 0;
    rep(i, M - 1) {
        ll p = (X[i + 1] + N - X[i]) % N;
        ll m = (X[i] + N - X[i + 1]) % N;
        ...
        if (p <= m) {
            if (X[i + 1] > X[i]) {
                tree.apply(X[i], X[i + 1], abs(p - m));
            } else {
                tree.apply(X[i], N, abs(p - m));
                tree.apply(0, X[i + 1], abs(p - m));
            }
        } else {
            if (X[i + 1] < X[i]) {
                tree.apply(X[i + 1], X[i], abs(p - m));
            } else {
                tree.apply(X[i + 1], N, abs(p - m));
                tree.apply(0, X[i], abs(p - m));
            }
        }
    }
    ll m = LINF;
