...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(j, N) tree.add(j, A[i][j]);
        rep(j, N - M + 1) A[i][j] = tree.sum(j, j + M);
    }
    rep(i, N - M + 1) {
        fenwick_tree<ll> tree(N);
        rep(j, N) tree.add(j, A[j][i]);
        rep(j, N - M + 1) A[j][i] = tree.sum(j, j + M);
    }
    SegmentTree2D seg(
        N - M + 1, N - M + 1, [](ll a, ll b) { return max(a, b); }, 0);
    rep(i, N - M + 1) rep(j, N - M + 1) seg.set(i, j, A[i][j]);
    seg.build();
