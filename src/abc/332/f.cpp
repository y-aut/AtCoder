...
mint e() { return 0; }
mint mp(pmm f, mint x) { return f.first * x + f.second; }
pmm comp(pmm f, pmm g) { return {f.first * g.first, f.first * g.second + f.second}; }
pmm id() { return {1, 0}; }

void solve() {
    LL(N, M);
    VM(A, N);
    lazy_segtree<mint, op, e, pmm, mp, comp, id> tree(A);
    rep(m, M) {
        LL(L, R, X);
        L--;
        tree.apply(L, R, {(mint)(R - L - 1) / (R - L), (mint)X / (R - L)});
    }
    vm ans;
    rep(i, N) ans.pb(tree.get(i));
    print(ans);
}
