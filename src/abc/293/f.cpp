...
ll calc(ll b, ll mask, ll dig) {
    ll cur = 0;
    rep(j, dig) if (mask >> j & 1) {
        double v = pow(double(b), j);
        if (v > LINF * 2) return LINF;
        cur += powll(b, j);
    }
    return cur;
}
