...
struct Node {
    ...
    Node(ll l, ll r, ll c) : left(l), right(r), count(c) {}
};

Node e() { return Node(0, 0, 0); }
Node op(Node a, Node b) { return e(); }
Node mapping(Node f, Node x) {
    if (f.left == LINF) return x;
    else if (f.left == -1) return f;
    else return Node(f.left, f.right, f.count + x.count);
}
Node composition(Node f, Node g) {
    if (f.left == LINF) return g;
    else if (f.left == -1) return f;
    else return Node(f.left, f.right, f.count + g.count);
}
Node id() { return Node(LINF, LINF, 0); }

void solve() {
    LL(Q, K);
    VPLL(P, Q);
    set<ll> xs;
    repi(i, x, P) if (i == 1) xs.insert(x);
    vll xv;
    repi(i, xs) xv.pb(i);
    umll xm;
    rep(i, xv.size()) xm[xv[i]] = i;
    ll X = xv.size();
    lazy_segtree<Node, op, e, Node, mapping, composition, id> tree(X);
    set<ll> inds;

    rep(i, X) tree.set(i, Node(-1, -1, 1));

    auto cut = [&](ll a, ll b) {
        if (a > b) swap(a, b);
        auto na = tree.get(a);
        tree.apply(na.left, a + 1, Node(na.left, a + 1, 0));
        tree.apply(a + 1, b, Node(-1, -1, 1));
        tree.apply(b, na.right, Node(b, na.right, -na.count));
    };
    auto link = [&](ll a, ll b) {
        if (a > b) swap(a, b);
        auto na = tree.get(a), nb = tree.get(b);
        if (na.left == -1) {
            na.left = a;
            na.right = a + 1;
        }
        ...
        tree.apply(nb.left, nb.right, Node(nb.left, nb.right, na.count));
        tree.apply(na.left, nb.right, Node(na.left, nb.right, 0));
    };
    auto size = [&](ll a) {
        auto na = tree.get(a);
        if (na.left == -1) {
        ...
        return tree.get(na.right - 1).count;
    };

    repi(i, x, P) {
        if (i == 1) {
            auto itr = inds.find(xm[x]);
            if (itr != inds.end()) {
                if (itr != inds.begin() && abs(xv[*itr] - xv[*prev(itr)]) <= K) {
                    cut(*itr, *prev(itr));
                }
                if (next(itr) != inds.end() && abs(xv[*itr] - xv[*next(itr)]) <= K) {
                    cut(*itr, *next(itr));
                }
                if (itr != inds.begin() && next(itr) != inds.end() && abs(xv[*prev(itr)] - xv[*next(itr)]) <= K) {
                    link(*prev(itr), *next(itr));
                }
                inds.erase(itr);
            } else {
                itr = inds.insert(xm[x]).first;
                if (itr != inds.begin() && next(itr) != inds.end() && abs(xv[*prev(itr)] - xv[*next(itr)]) <= K) {
                    cut(*prev(itr), *next(itr));
                }
                if (itr != inds.begin() && abs(xv[*itr] - xv[*prev(itr)]) <= K) {
                    link(*itr, *prev(itr));
                }
