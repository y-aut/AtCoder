...
void solve() {
    ...
    vll nums(1e6 + 2);
    repi(i, A) nums[i]++;
    repd(i, nums.size() - 1) nums[i] += nums[i + 1];
    ll ans = 0;
    ll prev = -1, prev_c = 0;
    rep(i, N - 1) {
        if (A[i] == prev) prev_c++;
        else prev = A[i], prev_c = 1;
        rep(j, 2, B) md[j][A[i] % j]--;
        if (A[i] < B) {
            ll tmp = acc[i + 1];
            rep(j, 1, A[i]) tmp -= j * md[A[i]][j];
            ans += tmp / A[i];
        } else {
            for (ll j = A[i]; j <= 1e6 + 1; j += A[i]) {
                ans += nums[j];
            }
            ans -= prev_c;
        }
