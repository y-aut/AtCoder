...
void solve() {
    ...
    segtree<ll, op, e> tree(W);
    ll last_h = hwd[0][0], last_i = 0;
    rep(i, 1, N) {
        if (last_h != hwd[i][0]) {
            rep(j, last_i, i) {
                tree.set(wm[hwd[j][1]], min(tree.get(wm[hwd[j][1]]), hwd[j][2]));
            }
            last_h = hwd[i][0];
            last_i = i;
        }
        if (tree.prod(0, wm[hwd[i][1]]) < hwd[i][2]) {
            Yes;
            return;
        }
    }
