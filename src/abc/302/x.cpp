...
struct UnionFindUndo {
    ...
    bool unite(int x, int y) {
        ...
        history_tree.push(tree[x]);
        history_tree.push(tree[y]);
        if (x == y) {
            if (tree[x]) tree[x] = false, tree_cnt--;
            return (false);
        }
        if (data[x] > data[y]) swap(x, y);
        data[x] += data[y];
        data[y] = x;
        if (tree[x] || tree[y]) tree_cnt--;
    ...
    void undo() {
        auto y = history.top().first;
        data[y] = history.top().second;
        bool a = tree[y] = history_tree.top();
        history.pop();
        history_tree.pop();
        auto x = history.top().first;
        data[x] = history.top().second;
        bool b = tree[x] = history_tree.top();
        history.pop();
        history_tree.pop();
        if (x == y) {
            if (tree[x]) tree_cnt++;
...
void solve() {
    ...
    UnionFindUndo uf(N);
    vll ans(N - 1);
    uf.unite(AB[0].first, AB[0].second);
    rep(i, 1, et.size()) {
        if (tree.get_depth(et[i - 1]) < tree.get_depth(et[i])) {
            uf.unite(AB[et[i]].first, AB[et[i]].second);
            ans[et[i] - 1] = N - uf.tree_cnt;
        } else {
            uf.undo();
        }
    }
