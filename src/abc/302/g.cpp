...
bool all_0(const vvll &A) {
    repi(i, A) repi(j, i) if (j) return false;
    return true;
}
...
void solve() {
    ...
    rep(i, 4) {
        ...
        vll b{(i + 1) % 4, (i + 2) % 4, i};
        rep(j, 2) {
            ll m = min({cnt[a[0]][b[0]], cnt[a[1]][b[1]], cnt[a[2]][b[2]]});
            cnt[a[0]][b[0]] -= m;
            cnt[a[1]][b[1]] -= m;
            cnt[a[2]][b[2]] -= m;
            ans += m * 2;
    ...
    vll p{0, 1, 2, 3};
    do {
        v<ll *> t{&cnt[p[0]][p[1]], &cnt[p[1]][p[2]], &cnt[p[2]][p[3]], &cnt[p[3]][p[0]]};
        ll m = min({*t[0], *t[1], *t[2], *t[3]});
        rep(i, 4) { *t[i] -= m; }
        ans += m * 3;
