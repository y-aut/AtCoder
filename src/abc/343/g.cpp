...
void solve() {
    ...
    rep(i, N) rep(j, N) {
        if (i == j) continue;
        RH tl(S[i].back()), hd(S[j].front());
        while (tl.size < min(S[i].size(), S[j].size())) {
            if (tl == hd) ol[i][j] = tl.size;
            tl.push_front(S[i][S[i].size() - tl.size - 1]);
            hd.push_back(S[j][hd.size]);
        }
    }
    debug(S);
    debug(ol);
    ...
    rep(i, 1LL << N) {
        rep(j, N) {
            ...
            rep(k, N) {
                ...
                chmax(dp[i | 1LL << j][j], dp[i][k] + ol[k][j]);
            }
        }
    }
    ll m = 0;
    rep(i, N) chmax(m, dp.back()[i]);
    ll ans = 0;
    rep(i, N) ans += S[i].size();
    print(ans - m);
}
