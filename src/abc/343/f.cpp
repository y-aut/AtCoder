...
public:
    ...
    void merge_to(const mp &a, const mp &b, mp &c) {
        repi(i, a) c[i.first] = i.second;
        repi(i, b) {
            if (c.count(i.first)) c[i.first] += i.second;
            else c[i.first] = i.second;
        }
    }
    ...
    void update(ll n) {
        ...
        update(child);
        update(child + 1);
        merge_to(seg[child], seg[child + 1], seg[n]);
    }
    ...
    Segtree(ll N, vll &A) {
        ...
        while (len < N) len <<= 1;
        seg.resize(len * 2 - 1);
        rep(i, A.size()) seg[len - 1 + i][A[i]] = 1;
        update(0);
    }

    void set(ll n, ll v) {
        ll old = seg[len - 1 + n].begin()->first;
        if (old == v) return;
        ll pos = len - 1 + n;
        while (true) {
            if (seg[pos][old] == 1) {
    ...
    pair<pll, pll> get(ll l, ll r, ll index, ll left, ll right) {
        if (l >= right || r <= left) return {{-1, 0}, {-2, 0}};
        if (l <= left && r >= right) {
            if (seg[index].size() >= 2) {
                return {*prev(seg[index].end()), *prev(prev(seg[index].end()))};
            } else {
                return {*prev(seg[index].end()), {-1, 0}};
            }
        }
        auto v1 = get(l, r, index * 2 + 1, left, (left + right) / 2);
        auto v2 = get(l, r, index * 2 + 2, (left + right) / 2, right);
        if (v1.first.first < v2.first.first) swap(v1, v2);
        if (v1.first.first == v2.first.first) {
            if (v1.second.first == v2.second.first) {
                return {{v1.first.first, v1.first.second + v2.first.second},
                        {v1.second.first, v1.second.second + v2.second.second}};
            } else if (v1.second.first > v2.second.first) {
                return {{v1.first.first, v1.first.second + v2.first.second}, v1.second};
            } else {
                return {{v1.first.first, v1.first.second + v2.first.second}, v2.second};
            }
        } else {
            if (v1.second.first == v2.first.first) {
                return {v1.first, {v1.second.first, v1.second.second + v2.first.second}};
            } else if (v1.second.first > v2.first.first) {
                return v1;
            } else {
...
void solve() {
    ...
    rep(q, Q) {
        ...
        if (n == 1) {
            ...
            auto v = tree.get(a, b, 0, 0, tree.len);
            print(v.second.second);
        }
    }
}
