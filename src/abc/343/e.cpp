...
inline ll len(const pll &a) { return a.second - a.first; }
...
bool is_ok(ll V1, ll V2, ll V3, ll a, ll b, ll c, ll d, ll e, ll f) {
    ll v3 = len(overlap(overlap({0, 7}, {a, a + 7}), {d, d + 7})) *
            len(overlap(overlap({0, 7}, {b, b + 7}), {e, e + 7})) *
            len(overlap(overlap({0, 7}, {c, c + 7}), {f, f + 7}));
    if (v3 != V3) return false;
    ll o12 = len(overlap({0, 7}, {a, a + 7})) *
             len(overlap({0, 7}, {b, b + 7})) *
             len(overlap({0, 7}, {c, c + 7}));
    ll o23 = len(overlap({a, a + 7}, {d, d + 7})) *
             len(overlap({b, b + 7}, {e, e + 7})) *
             len(overlap({c, c + 7}, {f, f + 7}));
    ll o13 = len(overlap({0, 7}, {d, d + 7})) *
             len(overlap({0, 7}, {e, e + 7})) *
             len(overlap({0, 7}, {f, f + 7}));
    if (o12 + o23 + o13 - v3 * 3 != V2) return false;
    return true;
}
...
void solve() {
    ...
    rep(i, 8) rep(j, 8) rep(k, 8) {
        rep(l, -7, 15) rep(m, -7, 15) rep(n, -7, 15) {
            if (is_ok(V1, V2, V3, i, j, k, l, m, n)) {
                Yes;
                vll ans{0, 0, 0, i, j, k, l, m, n};
                print(ans);
                return;
            }
        }
