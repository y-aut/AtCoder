...
void solve() {
    ...
    vector<pair<pll, pll>> dp(K + 1);
    dp[0].second.first = -1;
    rep(i, 1, K + 1) dp[i].first.first = dp[i].second.first = -1;
    rep(i, N) {
        vector<pair<pll, pll>> nxt(K + 1);
        nxt[0].first.first = nxt[0].second.first = -1;
        rep(j, K) {
            nxt[j + 1] = move(dp[j]);
        }
        rep(j, K + 1) {
            ll tmp = -1;
            if (dp[j].first.first != -1 && dp[j].first.first != CV[i].first) {
                tmp = dp[j].first.second + CV[i].second;
            } else if (dp[j].second.first != -1) {
                tmp = dp[j].second.second + CV[i].second;
            }
            if (tmp == -1) continue;
            if (tmp > nxt[j].first.second) {
                if (nxt[j].first.first != CV[i].first) {
                    nxt[j].second = nxt[j].first;
                }
                nxt[j].first = {CV[i].first, tmp};
            } else if (CV[i].first != nxt[j].first.first && tmp > nxt[j].second.second) {
                nxt[j].second = {CV[i].first, tmp};
            }
        }
        dp = move(nxt);
    }

    ll ans = dp[K].first.second;
    if (ans == 0) ans = -1;
    print(ans);
}
