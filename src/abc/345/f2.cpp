...
void solve() {
    LL(N, M, K);
    VPLL(uv, M);
    repi(i, uv) i.first--, i.second--;

    vvpll edges(N);
    rep(i, uv.size()) {
        edges[uv[i].first].eb(uv[i].second, i + 1);
        edges[uv[i].second].eb(uv[i].first, i + 1);
    }
    ...
    vi on(N), vis(N);
    ll on_cnt = 0;
    auto dfs = [&](auto rc, pll c, ll pv) -> void {
        if (on_cnt == K) return;
        vis[c.first] = 1;
        repi(i, edges[c.first]) {
            if (vis[i.first]) continue;
            rc(rc, i, c.first);
            if (on_cnt == K) return;
        }
        if (!on[c.first] && c.second != -1) {
            ans.pb(c.second);
            on_cnt -= on[c.first] + on[pv];
            on[c.first] ^= 1;
            on[pv] ^= 1;
            on_cnt += on[c.first] + on[pv];
        }
