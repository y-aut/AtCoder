...
class Binomial {
public:
    v<_mint> fact, fact_inv;

    Binomial(const ll size) : fact(size + 5), fact_inv(size + 5) {
        fact[0] = fact[1] = 1;
        fact_inv[0] = fact_inv[1] = 1;

        for (ll i = 2; i < size + 5; i++) {
            fact[i] = fact[i - 1] * i;
            fact_inv[i] = fact_inv[i - 1] / _mint(i);
        }
    }

    /// nCk % mod を求める
    inline _mint nCk(const ll n, const ll k) const {
...
void solve() {
    ...
    rep(i, 1, N) {
        ...
        for (; j <= i; j++) {
            ll t = N - i - 1 - j * K;
            if (t < 0) break;
            mint c = bn.fact[t + i] * bn.fact_inv[t] * bn.fact_inv[j] * bn.fact_inv[i - j];
            p[i] = c - p[i];
        }
        if (j % 2 == 0) p[i] = -p[i];
        p[i] *= inv;
    }
    rep(i, N) print(p[i] - p[i + 1]);
}
