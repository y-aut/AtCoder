...
void solve() {
    ...
    rep(use, 1LL << N) {
        ll area = 0;
        vll perm;
        for (ll i = 0, mask = 1; i < N; i++, mask <<= 1) {
            if (use & mask) {
                area += AB[i].first * AB[i].second;
                perm.pb(i);
            }
        }
        ...
        do {
            rep(mask, 1LL << perm.size()) {
                bool failed = false;
                set<ll> rest;
                rep(i, H * W) rest.insert(i);
                rep(i, perm.size()) {
                    ll width = AB[perm[i]].first, height = AB[perm[i]].second;
                    if (mask & (1LL << i)) {
                        if (width == height) {
                            failed = true;
                            break;
