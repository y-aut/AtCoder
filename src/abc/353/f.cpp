...
ll f(pll s, pll t) {
    bool ss = (s.first / K + s.second / K) % 2 == 0;
    bool ts = (t.first / K + t.second / K) % 2 == 0;
    if (s.first == t.first) {
        if (s.second == t.second) return 0;
        if (ss && ts) {
            ll big = abs(s.second / K - t.second / K) / 2;
            return t.second - s.second - big * 2;
        } else if (ss && !ts) {
            if (s.second < t.second) {
                return f(s, {t.first, (t.second / K) * K - 1}) + 1;
            } else {
                return f(s, {t.first, (t.second / K) * K + 3}) + 1;
            }
        } else if (!ss && ts) {
            return f(t, s);
        } else {
            ll small = abs(s.second / K - t.second / K) / 2;
            return small * 4;
        }
    } else {
        return f({s.second, s.first}, {t.second, t.first});
    }

    if (ss) {
        if (ts) {
            if (t.first % K == 0 && s.first < t.first) {
                return f(s, {t.first - 1, t.second}) + 1;
            } else if (t.first % K == K - 1 && s.first < t.first) {
            }
        }
    } else if (ts) {
        return f(t, s);
    } else {
        s.first = (s.first / K) * K + 1;
        s.second = (s.second / K) * K + 1;
        t.first = (t.first / K) * K + 1;
        t.second = (t.second / K) * K + 1;
        if (s.first == t.first || s.second == t.second) return f(s, t);

        ll ans = 0;
        auto m = min(abs(s.first - t.first), abs(s.second - t.second));
        if (abs(s.first - t.first) != abs(s.second - t.second)) {
            ans += f(s, {s.first + m, s.second});
        }
        ans += m / 3 * 2;
        return ans;
    }
