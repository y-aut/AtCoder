...
void solve() {
    ...
    ll ans = 0;
    auto scc = graph.scc();
    repi(g, scc) ans += g.size() * (g.size() - 1);
    ll G = scc.size();
    vll gn(N);
    rep(i, G) repi(j, scc[i]) gn[j] = i;
    v<usll> gedges(G);
    rep(i, N) repi(j, edges[i]) if (gn[i] != gn[j]) gedges[gn[i]].insert(gn[j]);
    ush<pll> e;
    v<usll> reachable(G);
    vb vis(G);
    auto dfs = [&](auto rc, ll v) -> void {
        ...
        repi(i, gedges[v]) {
            rc(rc, i);
            reachable[v].insert(i);
            repi(j, reachable[i]) reachable[v].insert(j);
        }
        vis[v] = true;
    };
    rep(i, G) {
    ...
    rep(i, G) repi(j, reachable[i]) ans += scc[i].size() * scc[j].size();
    print(ans - M);
}
