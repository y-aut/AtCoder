...
double mp(double f, double x) { return f + x; }
...
void solve() {
    ...
    PLL(S);
    VPLL(XY, N);
    // dp[n+1][k] <- dp[n][k+1] (k>=0)
    // dp[n+1][K-1] <- dp[n][i]
    // k+n
    lazy_segtree<double, op, e, double, mp, comp, id> dp(K);
    pll cur = S;
    dp.set(0, 0);
    rep(i, N) {
        // dp[i]
        auto tmp = dp.all_prod() + dist(cur, S) + dist(S, XY[i]);
        dp.apply(0, K, dist(cur, XY[i]));
        dp.set((K - 1 + i) % K, tmp);
        cur = XY[i];
    }
    print(dp.all_prod() + dist(cur, S));
}
