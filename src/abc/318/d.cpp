...
void solve() {
    ...
    ll ans = 0;
    rep(g, 1LL << N) {
        mcf_graph<ll, ll> gr(N + 2);
        rep(i, N) if (g >> i & 1) rep(j, N) if (!(g >> j & 1)) {
            gr.add_edge(i, j, 1, INF - edges[i][j]);
        }
        rep(i, N) if (g >> i & 1) gr.add_edge(N, i, 1, 0);
        rep(j, N) if (!(g >> j & 1)) gr.add_edge(j, N + 1, 1, 0);
        auto res = gr.flow(N, N + 1);
        chmax(ans, INF * res.first - res.second);
    }
    print(ans);
}
