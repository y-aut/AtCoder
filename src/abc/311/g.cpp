...
vvll acc_x(const vvll &A) {
    vvll ans(A.size(), vll(A[0].size() + 1));
    rep(i, A.size()) {
        rep(j, A[0].size()) {
            ans[i][j + 1] = ans[i][j] + A[i][j];
        }
    }
    return ans;
}

vvll acc_y(const vvll &A) {
    vvll ans(A.size() + 1, vll(A[0].size()));
    rep(i, A.size()) {
        rep(j, A[0].size()) {
            ans[i + 1][j] = ans[i][j] + A[i][j];
        }
    }
    return ans;
}

ll sum2d(const vvll &accxy, ll sy, ll sx, ll ty, ll tx) {
    assert(0 <= sy && sy <= ty && ty <= accxy.size());
    assert(0 <= sx && sx <= tx && tx <= accxy[0].size());
    return accxy[ty][tx] - accxy[sy][tx] - accxy[ty][sx] + accxy[sy][sx];
}

void solve() {
    LL(N, M);
    VVLL(A, N, M);
    auto accxy = acc_x(acc_y(A));

    vvpll add(300);
    rep(i, N) rep(j, M) add[A[i][j] - 1].eb(i, j);

    vvll ng(N, vll(M));
    ll ans = 0;
    rep(n, 300) {
        auto ngacc = acc_x(acc_y(ng));
        repi(i, j, add[n]) {
            vll perm{0, 1, 2, 3};
            do {
                ll pi = i, pj = j, si = i + 1, sj = j + 1;
                repi(d, perm) {
                    if (d == 0) {
                        // down
                        while (si < N) {
                            if (sum2d(ngacc, si, pj, si + 1, sj)) break;
                            si++;
                        }
                    } else if (d == 1) {
                        // right
                        while (sj < M) {
                            if (sum2d(ngacc, pi, sj, si, sj + 1)) break;
                            sj++;
                        }
                    } else if (d == 2) {
                        // up
                        while (pi > 0) {
                            if (sum2d(ngacc, pi - 1, pj, pi, sj)) break;
                            pi--;
                        }
                    } else {
                        // left
                        while (pj > 0) {
                            if (sum2d(ngacc, pi, pj - 1, si, pj)) break;
                            pj--;
                        }
                    }
                }
                if (chmax(ans, sum2d(accxy, pi, pj, si, sj) * (n + 1))) {
                    debug(ans);
                    debugs(i, j, pi, pj, si, sj);
                    debug(perm);
                }
            } while (next_permutation(all(perm)));
        }
        repi(i, j, add[n]) ng[i][j] = 1;
    }
    print(ans);
}
