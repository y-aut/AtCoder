...
void solve() {
    ...
    vvll nodes(N);
    rep(i, N) nodes[A[i]].pb(i);
    mint ans = 0;
    auto f = [&](auto rc, ll c, um<ll, vll> &children, ll v) -> mint {
        vm res;
        repi(i, children[v]) {
            res.pb(rc(rc, c, children, i));
            debug(i);
            debug(ans);
        }
        debug(v);
    ...
    rep(c, N) {
        ...
        auto p = tree.get_auxiliary_tree(nodes[c]);
        um<ll, vll> children;
        rep(i, 1, nodes[c].size()) children[p[nodes[c][i]]].pb(nodes[c][i]);
        debug(c);
        debug(children);
        f(f, c, children, nodes[c].front());
        debug(ans);
    }
    print(ans);
}
