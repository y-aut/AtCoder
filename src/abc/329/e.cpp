...
bool calc(string s, const string &T, bool hd, bool tl) {
    if (hd) {
        ...
        while (true) {
            ...
            rep(i, T.size()) {
                if (s.size() < i + 1) break;
                if (s.substr(0, i + 1) == T.substr(T.size() - i - 1)) {
                    s = s.substr(i + 1);
                    flg = true;
                    break;
                }
            }
            if (!flg || s.empty()) break;
        }
    }
    if (tl) {
        if (s.empty()) return true;
        while (true) {
            ...
            rep(i, T.size()) {
                if (s.size() < i + 1) break;
                if (s.substr(s.size() - i - 1) == T.substr(0, i + 1)) {
                    s = s.substr(0, s.size() - i - 1);
                    flg = true;
                    break;
                }
            }
...
void solve() {
    ...
    rep(i, N - M + 1) {
        bool flg = true;
        rep(j, M) if (S[i + j] != T[j]) BREAK(flg = false);
        if (flg) {
            if (i != last) {
                if (!calc(S.substr(last, i - last), T, last != 0, true)) EXIT(No);
            }
            i += M - 1;
            last = i + 1;
        }
    ...
    if (last == 0) No;
        ...
        if (!calc(S.substr(last, S.size() - last), T, true, false)) EXIT(No);
        Yes;
    }
}
