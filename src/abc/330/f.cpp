...
bool calc2(ll N, ll &K, const vpll &XY, ll v, const umll &p, const umll &nxtl, const umll &prvl) {
    ll xl = nxtl.at(-LINF), xu = prvl.at(LINF);
    ll xlcnt = 0, xucnt = 0;
    while (xu - xl > v) {
        if (xlcnt + p.at(xl) <= xucnt + p.at(xu)) {
            xlcnt += p.at(xl);
            auto nxt = nxtl.at(xl);
            if (xu - nxt <= v) {
                K -= xlcnt * (xu - xl - v);
...
bool calc(ll N, ll K, const vpll &XY, ll v, const umll &xp, const umll &yp,
          const umll &xnxt, const umll &xprv, const umll &ynxt, const umll &yprv) {
    return calc2(N, K, XY, v, xp, xnxt, xprv) && calc2(N, K, XY, v, yp, ynxt, yprv);
}
...
void solve() {
    ...
    set<ll> xs, ys;
    rep(i, N) {
        xs.insert(XY[i].first);
        ys.insert(XY[i].second);
    }

    umll xnxt, xprv, ynxt, yprv;
    ll last = -LINF;
    repi(i, xs) {
        xnxt[last] = i;
        xprv[i] = last;
    ...
    if (calc(N, K, XY, 0, xp, yp, xnxt, xprv, ynxt, yprv)) EXIT(print(0));
    ll lower = 0, upper = LINF;
    while (true) {
        if (lower + 1 >= upper) EXIT(print(upper));
        ll mid = (lower + upper) / 2;
        if (calc(N, K, XY, mid, xp, yp, xnxt, xprv, ynxt, yprv)) {
            upper = mid;
        } else {
            lower = mid;
        }
