...
ll ind(const vll &v) {
    ll ans = 0, now = 1;
    rep(i, v.size()) ans += v[i] * now, now *= 10;
    return ans;
}

vll decode(ll v, ll sz) {
    vll ans(sz);
    rep(i, sz) ans[i] = v % 10, v /= 10;
    return ans;
}
...
void solve() {
    ...
    rep(i, N) {
        ...
        repi(j, v, dp) {
            auto tmp = decode(j, K);
            rep(k, K) tmp[k] = min(tmp[k] + A[i][k], P);
            ll d = ind(tmp);
            if (nxt.count(d)) chmin(nxt[d], v + C[i]);
            else nxt[d] = v + C[i];
        }
        dp = move(nxt);
    }
    ll d = ind(vll(K, P));
    if (dp.count(d)) print(dp[d]);
    else print(-1);
}
