...
vs rotate(const vs &s) {
    vs ans(4, string(4, '_'));
    rep(i, 4) rep(j, 4) ans[j][3 - i] = s[i][j];
    return ans;
}
...
void solve() {
    ...
    rep(r, 1 << 6) {
        ...
        rep(i, 3) {
            ll d = r >> (i * 2) & 0b11;
            rep(j, d) R[i] = rotate(R[i]);
        }
        do {
            vvb board(4, vb(4));
            auto place = [&](ll p) -> bool {
                rep(i, 4) rep(j, 4) if (!board[i][j]) {
                    rep(k, 4) rep(l, 4) if (R[p][k][l] == '#') {
                        rep(m, 4) rep(n, 4) if (R[p][m][n] == '#') {
                            ll ni = i - k + m, nj = j - l + n;
                            if (!(0 <= ni && ni < 4 && 0 <= nj && nj < 4)) return false;
            ...
            if (place(ord[0]) && place(ord[1]) && place(ord[2])) {
                bool ok = true;
                rep(i, 4) rep(j, 4) ok &= board[i][j];
                if (ok) {
                    Yes;
                    return;
                }
