...
#include <library/suisen/library/number/garner.hpp>

void solve() {
    vll p{4, 9, 5, 7, 11, 13, 17, 19, 23};
    vll A;
    ll offset = 0;
    repi(i, p) {
        rep(j, i) A.pb((j + 1) % i + offset + 1);
        offset += i;
    }
    ll M = A.size();
    print(M);
    ...
    int new_m = 1;
    offset = 0;
    rep(i, M) if (B[i] == offset + 1) {
        ll cp = p[eq.size()];
        eq.eb(cp - (i - offset), cp);
        offset += cp;
        new_m *= cp;
    }
    print(suisen::garner(eq, new_m));
}
