...
ll find_unit(const string &s) {
    auto z = z_algorithm(s);
    rep(i, 1, s.size()) {
        if (s.size() % i == 0 && i + z[i] == s.size()) return i;
    }
    return s.size();
}
...
void solve() {
    ...
    ll L = 0;
    repi(i, S) L += i.size();
    vll units, ucnt, unums;
    um<string, ll> unum;
    repi(i, S) {
        units.pb(find_unit(i));
        auto unit = i.substr(0, units.back());
        if (!unum.count(unit)) unum.emplace(unit, unum.size()), ucnt.pb(0);
        unums.pb(unum[unit]);
        ucnt[unums.back()]++;
    }
    v<set<ll>> ex(ucnt.size());
    vll ans(N);
    v<umll> memo(ucnt.size());
    rep(i, N) {
        ll r = S[i].size() / units[i];
        ll cur = memo[unums[i]][r];
        rep(k, cur + 1, LINF) {
            if (!ex[unums[i]].count(r * k)) {
                ex[unums[i]].insert(r * k);
                ans[i] = k;
                memo[unums[i]][r] = k;
                break;
            }
