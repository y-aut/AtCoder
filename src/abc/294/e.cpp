...
int main() {
    ...
    ll ans = 0;
    ll now1 = 0, now2 = 0, ind1 = 0, ind2 = 0;
    while (ind2 < N2) {
        if (now1 <= now2) {
            for (; now1 <= now2 && ind1 < N1; ind1++) {
                now1 += P1[ind1].second;
                if (ind2 > 0 && P1[ind1].first == P2[ind2 - 1].first) {
                    ans += max(0LL, min(now1, now2) - max(now1 - P1[ind1].second, now2 - P2[ind2 - 1].second));
                }
            }
        } else {
            swap(N1, N2);
