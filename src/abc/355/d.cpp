...
void solve() {
    ...
    VPLL(lr, N);
    set<ll> xs;
    repi(i, lr) xs.insert(i.first), xs.insert(i.second);
    umll xm;
    ll index = 0;
    repi(i, xs) xm[i] = index++;
    repi(i, lr) i.first = xm[i.first], i.second = xm[i.second];
    sort(all(lr), [](const pll &a, const pll &b) {
        return a.second < b.second;
    });
    fenwick_tree<ll> rs(xs.size());
    ll ans = 0;
    repi(i, lr) {
        ans += rs.sum(i.first, xs.size());
        rs.add(i.second, 1);
    }
    print(ans);
}
