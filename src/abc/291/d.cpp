...
void solve() {
    ...
    rep(i, 1, N) {
        pmm nxt{0, 0};
        if (AB[i].first != AB[i - 1].first) nxt.first += dp.first;
        if (AB[i].first != AB[i - 1].second) nxt.first += dp.second;
        if (AB[i].second != AB[i - 1].first) nxt.second += dp.first;
        if (AB[i].second != AB[i - 1].second) nxt.second += dp.second;
        dp = nxt;
    }
    print(dp.first + dp.second);
}
// 