...
void solve() {
    ...
    rep(i, 1LL << K) {
        bool flg = true;
        rep(j, K) {
            if ((i >> j & 1) != (i >> (K - j - 1) & 1)) BREAK(flg = false);
        }
        if (flg) pal.pb(i);
    }
    debug(pal);
    vll qcnt(N + 1);
    rep(i, N) qcnt[i + 1] = qcnt[i] + (S[i] == '?');
    vm dp(1LL << K);
    dp[0] = 1;
    rep(i, K - 1) {
        vm nxt(1LL << K);
        rep(j, 1LL << K) {
            ll nm = (j << 1) & mask;
            if (S[i] != 'B') nxt[nm] += dp[j];
            if (S[i] != 'A') nxt[nm | 1] += dp[j];
        }
        dp = move(nxt);
    }
    mint ans = 0;
    rep(i, K - 1, N) {
        ...
        debug(dp);
        repi(j, pal) {
            ans += dp[j] * mint(2).pow(qcnt.back() - qcnt[i + 1]);
            dp[j] = 0;
        }
    }
    print(mint(2).pow(qcnt.back()) - ans);
}
