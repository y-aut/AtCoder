...
void solve() {
    ...
    CentroidDecomposition tree(edges);
    VLL(A, N);
    repi(i, A) i--;

    ll ans = 0;
    auto count = [&](auto rc, ll v, ll par, const vb &dead, umll &res) -> void {
        res[A[v]]++;
        repi(i, edges[v]) {
            if (i == par || dead[i]) continue;
            rc(rc, i, v, dead, res);
    ...
    auto g = [&](auto rc, ll root, ll v, ll par, ll depth, const vb &dead, const umll &cnt, const umll &cnt_all) -> void {
        if (A[v] == A[root]) ans += depth;
        ans += depth * (cnt_all.at(A[v]) - cnt.at(A[v]));
        repi(i, edges[v]) {
            if (i == par || dead[i]) continue;
            rc(rc, root, i, v, depth + 1, dead, cnt, cnt_all);
        }
    };
    auto f = [&](ll root, const vb &dead) -> void {
        um<ll, umll> cnt;
        umll cnt_all;
        repi(c, edges[root]) {
            if (dead[c]) continue;
            umll res;
            count(count, c, root, dead, res);
            cnt[c] = res;
            repi(i, res) cnt_all[i.first] += i.second;
        }
        repi(i, cnt) {
            g(g, root, i.first, root, 1, dead, i.second, cnt_all);
        }
    };
    tree.perform(f);
    print(ans);
