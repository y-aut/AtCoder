...
pll op(const pll &a, const pll &b) { return a.second <= b.second ? a : b; }
pll e() { return {0, LINF}; }
...
void solve() {
    ...
    vpll init;
    rep(i, N) init.eb(i, 3 * A[i]);
    segtree<pll, op, e> d(init);
    rep(c, N - 2) {
        auto res = d.all_prod();
        ans += res.second;
        d.set(res.first, {res.first, d.get(res.first).second + A[res.first] * 2});
    }
    print(ans);
}
