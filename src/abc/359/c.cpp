...
ll f(ll x, ll y, ll tx, ll ty) {
    if (x > tx) return f(tx, ty, x, y);
    if (x == tx) return abs(y - ty);
    if (y > ty) return f(x, y, tx, y + y - ty);
    if (y == ty) {
        if ((tx - x) % 2 == 0) return (tx - x) / 2;
        if ((x + y) % 2 == 0) return (tx - x) / 2;
        return (tx - x) / 2 + 1;
    }
    if ((x + y) % 2 == 1) return f(x, y + 1, tx, ty) + 1;
    if (tx - x <= ty - y) return ty - y;
    return f(x + ty - y, ty, tx, ty) + ty - y;
}
