...
double calc(const vll &Q) {
    double ans = 0, tmp = 0;
    rrep(i, Q.size()) ans += pow(0.9, Q.size() - i) * Q[i - 1];
    rrep(i, Q.size()) tmp += pow(0.9, Q.size() - i);
    return ans / tmp - 1200 / sqrt(Q.size());
}
...
void solve() {
    ...
    rep(i, 1LL << N) {
        vll Q;
        for (ll j = 0, mask = 1; j < N; j++, mask <<= 1) {
            if (i & mask) {
                Q.pb(P[j]);
            }
            chmax(ans, calc(Q));
