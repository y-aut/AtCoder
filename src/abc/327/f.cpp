...
void solve() {
    ...
    lazy_segtree<ll, op, e, ll, mapping, composition, id> seg(200010);
    ll hd = 0, ans = 0;
    rep(i, N) {
        seg.apply(max(0LL, TX[i].second - W + 1), TX[i].second + 1, 1);
        while (TX[hd].first <= TX[i].first - D) {
            seg.apply(max(0LL, TX[hd].second - W + 1), TX[hd].second + 1, -1);
            hd++;
        }
