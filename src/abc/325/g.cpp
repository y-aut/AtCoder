...
vvll memo;
ll calc(const string &S, ll K, ll l, ll r) {
    if (l >= r) return 0;
    if (memo[l][r] != -1) return memo[l][r];
    memo[l][r] = r - l;
    if (S[l] == 'o') {
        rep(i, l + 1, r) {
            if (S[i] == 'f' && calc(S, K, l + 1, i) == 0) {
                chmin(memo[l][r], max(0LL, calc(S, K, i + 1, r) - K));
            }
        }
    }
    rep(i, l + 1, r) {
        if (S[i] == 'o') {
            chmin(memo[l][r], calc(S, K, l, i) + calc(S, K, i, r));
        }
    }
    return memo[l][r];
}
...
int main() {
    ...
    memo = vvll(S.size(), vll(S.size() + 1, -1));
    print(calc(S, K, 0, S.size()));
