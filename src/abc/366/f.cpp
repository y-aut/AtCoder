...
void solve() {
    ...
    if (K == 1) {
        ll ans = 0;
        repi(a, b, AB) chmax(ans, a + b);
        print(ans);
        return;
    }
    sort(all(AB), [](pll p1, pll p2) { return (p1.first - 1) * p2.second < (p2.first - 1) * p1.second; });
    debug(AB);
    ll F = K / 2, B = K - F;
    vv<umll> fw(N + 1, v<umll>(F + 1));
    fw[0][0][1] = 0;
    rep(i, N) {
        rep(j, F) fw[i + 1][j] = move(fw[i][j]);
        fw[i + 1][F] = fw[i][F];
        repd(j, F) repi(a, b, fw[i + 1][j]) {
            ll na = a * AB[i].first;
            ll nb = AB[i].first * b + AB[i].second;
            if (!fw[i + 1][j + 1].count(na)) fw[i + 1][j + 1][na] = nb;
            else chmax(fw[i + 1][j + 1][na], nb);
        }
    }
    vv<umll> bw(N + 1, v<umll>(B + 1));
    bw[N][0][1] = 0;
    repd(i, N) {
        rep(j, B) bw[i][j] = move(bw[i + 1][j]);
        bw[i][B] = bw[i + 1][B];
        repd(j, B) repi(a, b, bw[i][j]) {
            ll na = a * AB[i].first;
            ll nb = AB[i].first * b + AB[i].second;
            if (!bw[i][j + 1].count(na)) bw[i][j + 1][na] = nb;
            else chmax(bw[i][j + 1][na], nb);
        }
    }
    ll ans = 0;
    rep(i, N) {
        ll m = 0;
        repi(a, b, fw[i][F]) chmax(m, a + b);
    }
    print(ans);
    ...
    // rep(i, K) {
    //     auto &&cur = AB[i];
    //     pll nxt{now.first * cur.first, now.second * cur.first + cur.second};
    //     now = nxt;
    // }
