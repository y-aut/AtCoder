...
int main()
{
    ...
    reps(k, 1, K) {
        ...
        ll j_max = k;
        rep(i, used+1) {
            // ans[j] > ans[k-1] - A.at(i) なる最小の j を見つける
            auto target = ans[k-1] - A.at(i);
            auto j = upper_bound(ans, ans + k, target) - ans;
            if (j < j_max) {
                val = min(val, ans[j] + A.at(i));
                j_max = j;
            }
        }
        if (used < N-1 && A.at(used+1) <= val) {
            val = A.at(++used);
        }
        ans[k] = val;
    }
