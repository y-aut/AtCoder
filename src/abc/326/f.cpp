...
void get_set(vll &A, vll &vals, vll &ind) {
    vpll ans;
    rep(i, 1LL << A.size()) {
        ll val = 0;
        for (ll j = 0, mask = 1; j < A.size(); j++, mask <<= 1) {
            if (i & mask) val += A[j];
            else val -= A[j];
        }
        ans.eb(val, i);
...
bool solve(vll &A, ll v, vb &ans) {
    vll u, l;
    rep(i, A.size() / 2) u.pb(A[i]);
    rep(i, A.size() / 2, A.size()) l.pb(A[i]);
    vll uvals, uind, lvals, lind;
    get_set(u, uvals, uind);
    get_set(l, lvals, lind);
    rep(i, uvals.size()) {
        auto rest = v - uvals[i];
        auto itr = lower_bound(all(lvals), rest) - lvals.begin();
        if (0 <= itr && itr < lvals.size() && lvals[itr] == rest) {
            for (ll j = 0, mask = 1; j < u.size(); j++, mask <<= 1) {
                ans.pb(uind[i] & mask);
            }
            for (ll j = 0, mask = 1; j < l.size(); j++, mask <<= 1) {
                ans.pb(lind[itr] & mask);
            }
            return true;
        }
    }
...
int main() {
    ...
    vb ans1, ans2;
    if (solve(a, Y, ans1) && solve(b, X, ans2)) {
        Yes;
        bool dir = true;
        rep(i, N) {
            if (i % 2 == 0) {
                cout << (ans1[i / 2] ^ dir ? "R" : "L");
                dir = ans1[i / 2];
            } else {
                cout << (ans2[i / 2] ^ dir ? "L" : "R");
                dir = ans2[i / 2];
            }
        }
        cout << endl;
