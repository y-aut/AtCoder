...
vb memo_ex(10010, false);
vm memo(10010, 0);
mint calc(ll N, vll &T, vll &cnt, ll t) {
    if (memo_ex[t]) return memo[t];
    if (t == 0) return memo_ex[t] = true, memo[t] = 1;
    mint ans = 0;
    rep(i, t) {
        if (cnt[t - i]) ans += calc(N, T, cnt, i) * cnt[t - i] / N;
    }
    return memo_ex[t] = true, memo[t] = ans;
}
