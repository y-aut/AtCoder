...
int main() {
    ...
    ll high = *max_element(all(p));

    vpll As;
    rep(i, M) As.eb(i, A[i]);
    sort(all(As), [](pll a, pll b) { return a.second > b.second; });

    rep(i, N) {
        if (p[i] == high) CONTINUE(print(0));
        ll cnt = 0;
        rep(j, M) {
            if (S[i][As[j].first] == 'o') continue;
            cnt++;
            p[i] += As[j].second;
            if (p[i] > high) BREAK(print(cnt));
        }
    }
