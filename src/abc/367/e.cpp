...
void solve() {
    ...
    rep(i, N) {
        ...
        if (cnum[j] != -1) {
            vll l;
            for (ll k = i; k != j; k = X[k]) l.pb(k);
            reverse(all(l));
            if (cnum[j] >= INF) {
                if (j == ls[cnum[j]].back()) {
                    repi(k, l) ls[cnum[j]].pb(k), cnum[k] = cnum[j], ind[k] = ls[cnum[j]].size() - 1;
                } else {
                    repi(k, l) ls[gi].pb(k), cnum[k] = gi, ind[k] = ls[gi].size() - 1;
                    gi++;
                }
            } else {
                repi(k, l) ls[gi].pb(k), cnum[k] = gi, ind[k] = ls[gi].size() - 1;
    ...
    vll B(N);
    auto f = [&](auto rc, ll v, ll k) -> ll {
        if (cnum[v] < INF) {
            return ls[cnum[v]][(ind[v] + k) % ls[cnum[v]].size()];
        }
        ll i = ind[v] - k;
        if (i >= 0) return ls[cnum[v]][i];
        k -= ind[v] + 1;
        v = X[ls[cnum[v]][0]];
        return rc(rc, v, k);
    };
