...
void solve() {
    ...
    repi(r, c, RC) r--, c--;
    sort(all(RC));
    segtree<pll, op, e> col(W);
    vpll from(N);
    rep(i, N) {
        auto [r, c] = RC[i];
        auto res = col.prod(0, c + 1);
        col.set(c, {res.first + 1, i});
        from[i] = {res.first + 1, res.second};
    }
    ll ans = 0;
    string path;
    path.reserve(H + W - 2);
    ll pos = 0;
    rep(i, N) if (chmax(ans, from[i].first)) pos = i;
    pll to{H - 1, W - 1};
    while (true) {
        pll sq;
        if (pos == -1) sq = {0, 0};
        else sq = RC[pos];
        rep(i, to.first - sq.first) path.pb('D');
        rep(i, to.second - sq.second) path.pb('R');
        to = sq;
        if (pos == -1) break;
        pos = from[pos].second;
    }
