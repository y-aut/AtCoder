...
void solve() {
    ...
    while (T--) {
        ...
        VLL(A, N);
        repi(i, A) i--;
        segtree<ll, opmin, emin> mnt(A);
        segtree<ll, opmax, emax> mxt(A);
        v<queue<ll>> pos(N);
        set<ll> req;
        rep(i, N) pos[A[i]].push(i);
        rep(i, N) if (!pos[i].empty()) req.insert(pos[i].back());
        vll ans;
        rep(i, N) {
            if (pos[A[i]].empty()) continue;
            if (pos[A[i]].size() == 1) {
                assert(!req.empty() && *req.begin() == i);
                req.erase(i);
                ans.pb(A[i] + 1);
                continue;
            }
            ...
            pos[A[i]].pop();
            if (ans.size() % 2 == 0) {
                ll res = mxt.prod(i + 1, *req.begin() + 1);
                if (res > A[i]) continue;
            } else {
                ll res = mnt.prod(i + 1, *req.begin() + 1);
                if (res < A[i]) continue;
            }
            ans.pb(A[i] + 1);
            req.erase(pos[A[i]].back());
            while (!pos[A[i]].empty()) {
                ll p = pos[A[i]].front();
                pos[A[i]].pop();
                mnt.set(p, LINF);
