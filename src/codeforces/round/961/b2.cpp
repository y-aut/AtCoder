...
void solve() {
    ...
    rep(t, T) {
        ...
        ll ans = 0;
        repi(num, c, cnt) {
            chmax(ans, min(c, M / num) * num);
        }
        for (auto itr = cnt.begin(); next(itr) != cnt.end(); itr++) {
            auto nxt = next(itr);
            if (itr->first + 1 != nxt->first) continue;
            ll j = min(nxt->second, M / nxt->first);
            ll i = min(itr->second, (M - j * nxt->first) / itr->first);
            ll c = i * itr->first + j * nxt->first;
            chmax(ans, c);
            ll d = itr->first - (M - c);
            if (d < 0) continue;
            if (itr->second - i >= d + 1 && j >= d) chmax(ans, M);
        }
        print(ans);
    }
}
