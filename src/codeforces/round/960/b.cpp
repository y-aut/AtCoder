...
bool check(const vll &a, ll x, ll y) {
    ll m = -LINF, mi = 0;
    ll acc = 0;
    rep(i, a.size()) {
        acc += a[i];
        if (chmax(m, acc)) mi = i + 1;
    }
    if (x != mi) return false;
    m = -LINF, mi = 0;
    acc = 0;
    repd(i, a.size()) {
        acc += a[i];
...
void solve() {
    ...
    rep(t, T) {
        LL(N, X, Y);
        vll ans;
        if (Y % 2 == 1 && Y > 1) ans.pb(-1);
        ll now = -1;
        while (ans.size() < Y - 1) ans.pb(now), now = -now;
        rep(i, Y - 1, X) ans.pb(1);
        rep(i, X, N) ans.pb((i - X) % 2 == 0 ? -1 : 1);
        print(ans);
        debug(check(ans, X, Y));
    }
}
