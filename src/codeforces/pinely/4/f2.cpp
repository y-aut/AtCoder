...
bool checkn(const set<ll>::iterator &itr, const set<ll>::iterator &end, ll n) {
    assert(itr != end);
    rrep(i, n) {
...
bool check3(const set<ll> &cur, const set<ll>::iterator &itr, const set<ll>::iterator &last) {
    auto i = cur.find(*itr);
    if (next(i) == cur.end()) return false;
    if (next(i, 2) == cur.end()) return false;
    if (next(i, 3) == cur.end()) return false;
    return *next(i, 3) <= *last;
}
...
void solve() {
    ...
    usll spec;
    rep(i, N - 6) {
        if (check(A[i], A[i + 3], A[i + 4]) && check(A[i + 1], A[i + 2], A[i + 5]) ||
            check(A[i], A[i + 3], A[i + 4]) && check(A[i + 1], A[i + 2], A[i + 5]) ||
            check(A[i], A[i + 3], A[i + 4]) && check(A[i + 1], A[i + 2], A[i + 5])) {
            spec.insert(i);
        }
    }
    ...
    set<ll> good, good1, good2, good3;
    auto add_goodn = [&](ll a1, ll a2, ll a3, ll a4, ll a5, ll a6, ll v) {
        if (check(a1, a4, a5) && check(a2, a3, a6)) good1.insert(v);
        if (check(a1, a3, a4) && check(a2, a5, a6)) good2.insert(v);
        if (check(a1, a5, a6) && check(a2, a3, a4)) good3.insert(v);
    };
    set<ll> cur;
    {
        rep(i, 2) {
    ...
    rep(i, 2, N) {
        ...
        }
        {
            ...
            tmp = itr;
            rep(j, 6) {
                if (checkn(tmp, cur.end(), 5)) {
                    add_goodn(Ap[*tmp].first, Ap[*next(tmp)].first, Ap[*next(tmp, 2)].first,
                              Ap[*next(tmp, 3)].first, Ap[*next(tmp, 4)].first, Ap[*next(tmp, 5)].first, *tmp);
                }
                if (tmp == cur.begin()) break;
                tmp = prev(tmp);
            }
        ...
        debug(good2);
        debug(good3);
        while (!good1.empty() || !good2.empty() || !good3.empty() || (!good.empty() && check3(cur, good.begin(), prev(good.end())))) {
            pos[st] = i;
            auto t = Aind[st++];
            auto itr = cur.find(t);
            assert(itr != cur.end());
