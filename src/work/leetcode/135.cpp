...
class Solution {
public:
    void track(vector<int> &ratings, int l, int r, int p, vector<int> &count) {
        count[p - l] = 1;
        for (int i = p - 1; i >= l; i--) {
    ...
    int partial(vector<int> &ratings, int l, int r) {
        if (r - l == 1) return 1;
        vector<int> count(r - l);
        if (ratings[l] < ratings[l + 1]) track(ratings, l, r, l, count);
        if (ratings[r - 1] < ratings[r - 2]) track(ratings, l, r, r - 1, count);
        for (int i = l + 1; i < r - 1; i++) {
            if (ratings[i] < min(ratings[i - 1], ratings[i + 1])) {
                track(ratings, l, r, i, count);
            }
        }
        for (int i = l; i < r; i++) {
            if (count[i - l] == -1) {
                if (i == l) count[i - l] = count[i - l + 1] + 1;
                else if (i == r - 1) count[i - l] = count[i - l - 1] + 1;
                else count[i - l] = max(count[i - l - 1], count[i - l + 1]) + 1;
            }
        }
    ...
    int candy(vector<int> &ratings) {
        ...
        for (int i = 1; i < ratings.size(); i++) {
            if (ratings[i] == ratings[i - 1]) {
                ans += partial(ratings, start, i);
                start = i;
                continue;
            }
        }
        ans += partial(ratings, start, ratings.size());
        return ans;
    }
};
