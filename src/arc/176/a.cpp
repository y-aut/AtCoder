...
void solve() {
    LL(N, M);
    VPLL(AB, M);
    repi(i, AB) i.first--, i.second--;
    vpll ans;
    usll start;
    repi(i, AB) {
        ll tmp = i.second - i.first;
        if (tmp < 0) tmp += N;
        if (start.count(tmp)) continue;
        start.insert(tmp);
        rep(j, N) ans.eb(j + 1, (tmp + j) % N + 1);
    }
    for (int i = 0; i < N && start.size() < M; i++) {
        if (start.count(i)) continue;
        start.insert(i);
        rep(j, N) ans.eb(j + 1, (i + j) % N + 1);
    }
    print(ans.size());
    print(ans);
}
