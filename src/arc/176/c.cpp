...
void solve() {
    LL(N, M);
    VVLL(ABC, M, 3);
    repi(i, ABC) i[0]--, i[1]--, i[2]--;

    mint ans = 1;
    vvpll edges(N, vpll());
    repi(i, ABC) {
        edges[i[0]].eb(i[1], i[2]);
        edges[i[1]].eb(i[0], i[2]);
    }
    repi(i, edges) {
        sort(all(i), [](const pll &a, const pll &b) {
    ...
    vll nums(N, -1);
    dsu uf(N);
    repi(i, ABC) uf.merge(i[0], i[1]);
    auto groups = uf.groups();
    repi(group, groups) {
        repi(i, group) {
            if (edges[i].size() >= 2) {
                ll prev_v = -1, prev = -1, must = -1;
                repi(e, edges[i]) {
                    if (must != -1) {
                        if (e.second == must) continue;
                        print(0);
                        return;
                    }
                    if (e.second == prev) {
                        ...
                        if (nums[prev_v] != -1 && nums[prev_v] != prev) {
                            print(0);
                            return;
                        }
                        nums[prev_v] = prev;
    ...
    repi(i, ABC) {
        if (nums[i[0]] != -1 && nums[i[1]] != -1) {
            if (i[2] != max(nums[i[0]], nums[i[1]])) {
                print(0);
                return;
            }
        } else if (nums[i[0]] != -1 || nums[i[1]] != -1) {
            if (i[2] < max(nums[i[0]], nums[i[1]])) {
                print(0);
                return;
            }
    ...
    repi(i, ABC) {
        if (nums[i[0]] == -1) {
            chmin(upper[i[0]], i[2] - 1);
        } else if (nums[i[1]] == -1) {
            chmin(upper[i[1]], i[2] - 1);
        }
    }
    ...
    vll rest_v;
    repi(i, rest) rest_v.pb(i);

    vll uppers;
    rep(i, N) {
        if (nums[i] == -1) uppers.pb(upper[i]);
    }
    sort(all(uppers));

    rep(i, uppers.size()) {
        ll c = upper_bound(all(rest_v), uppers[i]) - rest_v.begin();
        if (c - i < 0) {
            print(0);
            return;
        }
