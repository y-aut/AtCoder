...
void solve() {
    ...
    vector dp(3, vector(3, mint(0)));
    dp[1][2] = 1;
    rep(i, M) {
        vector nxt(3, vector(3, mint(0)));
        nxt[0][0] = dp[0][0] * (N - 4) * 2 + dp[0][1] + dp[0][2] + dp[1][0] + dp[2][0];
        rrep(j, 2) {
            nxt[0][j] = dp[0][0] * (N - 3) + dp[0][j] * (N - 3) + dp[0][3 - j] + dp[1][j] + dp[2][j];
            nxt[j][0] = dp[0][0] * (N - 3) + dp[j][0] * (N - 3) + dp[3 - j][0] + dp[j][1] + dp[j][2];
            nxt[j][3 - j] = dp[0][3 - j] * (N - 2) + dp[j][0] * (N - 2);
        }
        rep(j, 3) rep(k, 3) nxt[j][k] += dp[j][k] * (N - 2) * (N - 3) / 2;
        rep(j, 3) rep(k, 3) nxt[k][j] += dp[j][k];
        dp = move(nxt);
    }
    debug(dp);
    assert(dp[1][0] == dp[0][2]);
    assert(dp[2][0] == dp[0][1]);
    mint ans = 0, tmp = (N - 1) * N * (N + 1) / 6;
    rep(i, N - 1) {
        mint dif = abs(P[i] - P[i + 1]);
        tmp -= dif;
        ans += dif * ((i == 0 || i == N - 2 ? N - 3 : N - 4) * dp[0][0] * 2 + dp[1][2] + dp[2][1] + (i == 0 || i == N - 2 ? 1 : 2) * (dp[1][0] + dp[2][0]));
    }
    debug(ans);
    {
        mint dif = abs(P[0] - P[N - 1]);
        tmp -= dif;
        ans += dif * ((N - 3) * dp[0][0] * 2 + dp[0][1] + dp[0][2] + dp[1][0] + dp[2][0]);
    }
    debug(ans);
    rep(i, N - 3) {
        mint dif = abs(P[0] - P[i + 2]);
        tmp -= dif;
        ans += dif * ((N - 4) * dp[0][0] * 2 + dp[0][1] + dp[1][0] + (dp[0][2] + dp[2][0]) * 2);
    }
    rep(i, N - 3) {
        mint dif = abs(P[i + 1] - P[N - 1]);
        tmp -= dif;
        ans += dif * ((N - 4) * dp[0][0] * 2 + dp[0][2] + dp[2][0] + (dp[0][1] + dp[1][0]) * 2);
    }
    debug(ans);
    ans += tmp * ((N - 5) * dp[0][0] * 2 + (dp[0][1] + dp[1][0]) * 2 + (dp[0][2] + dp[2][0]) * 2);
    print(ans);
}
