...
void solve() {
    ...
    rep(i, 3) rep(j, N - 1) cnt[i][j + 1] += cnt[i][j];
    mint ans = 0;
    SquareMatrix<mint, 3> mat;
    rep(k, 1, N) {
        mat[0][0] = 2 * (k - 2) + 1 + (N - 2) * (N - 3) / 2;
        mat[0][1] = k - 1;
        mat[0][2] = 0;
        mat[1][0] = 2 * (N - k);
        mat[1][1] = N - 1 + (N - 2) * (N - 3) / 2;
        mat[1][2] = 2 * k;
        mat[2][0] = 0;
        mat[2][1] = N - k - 1;
        mat[2][2] = 2 * (N - k - 2) + 1 + (N - 2) * (N - 3) / 2;
        mat = mat.pow(M);
        ans += mat[1][0] * cnt[0][k - 1] + mat[1][1] * cnt[1][k - 1] + mat[1][2] * cnt[2][k - 1];
    }
    print(ans);
}
