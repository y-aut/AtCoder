...
int main() {
    ...
    ll maxn = prev(nums.end())->first;

    // dp[i]: 次の数字で，合成したものが i 個あるものの通り数
    um<ll, mint> dp;
    dp[0] = 1;
    ...
    for (ll n = 1;; n++) {
        um<ll, mint> nxt;
        repi(i, dp) {
            for (ll j = 0; j <= i.first / 2; j++) {
                nxt[j + nums[n]] += i.second;
            }
        }
        dp = move(nxt);
        if (n > maxn && dp.size() == 1 && dp.begin()->first == 0) break;
    }

    print(dp.begin()->second);
