...
void solve() {
    ...
    while (T--) {
        ...
        repi(i, B) i--;
        vvll target(N);
        rep(i, N) target[B[i]].pb(i);
        queue<ll> q;
        vb done(N);
        rep(i, N) if (A[i] == B[i]) q.push(i), done[i] = true;
        while (!q.empty()) {
            auto v = q.front();
            q.pop();
            auto pos = lower_bound(all(target[A[v]]), v - K) - target[A[v]].begin();
            rep(i, pos, target[A[v]].size()) {
                ll t = target[A[v]][i];
                if (t > v + K) break;
                if (done[t]) continue;
                q.push(t);
