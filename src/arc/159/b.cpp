...
ll next_factor(ll n, ll start, ll end) {
    if (start <= 2 && n % 2 == 0) return 2;
    if (start % 2 == 0) start++;
...
int main()
{
    ...
    while (a >= 1) {
        ...
        if (dif != 0) {
            // dif の素因数を列挙
            auto root = (ll)ceil(sqrt(dif));
            auto f = next_factor(dif, 2, root);
            if (f == -1) {
                // dif が素数
                // a が dif の倍数になるまで進める
                auto mod = a % dif;
                ...
                while (f != -1) {
                    dif /= f;
                    root = (ll)ceil(sqrt(dif));
                    f = next_factor(dif, 2, root);
                    if (f != -1) flist.pb(f);
                    else flist.pb(dif);
                }
                auto minMod = LINF;
