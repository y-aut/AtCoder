...
void solve() {
    ...
    vector<set<ll>> pos(10);
    rep(i, N) pos[A[i]].insert(i);

    vector<vector<segtree<ll, op, e>>> dp(10, vector<segtree<ll, op, e>>(10, segtree<ll, op, e>(N)));
    vll last(10, -1);
    ...
    rep(i, N) {
        ...
        rep(n, 10) {
            ...
            ll third = A[i] * 2 - n;
            if (third < 0 || 10 <= third) continue;
            if (pos[third].empty()) continue;
            dp[n][A[i]].set(last[n], min(dp[n][A[i]].get(last[n]), *pos[third].begin()));
        }
        last[A[i]] = i;
    }
