    ...
    rep(t, T) {
        ...
        rep(i, N) {
            ...
            if (p == 0) {
                ...
                if (abs(A[p] - B[i]) < abs(A[p - 1] - B[i])) {
                    pos[i] = p;
                    if (p == N - 1 || abs(A[p - 1] - B[i]) <= abs(A[p + 1] - B[i])) {
                        second[i] = p - 1;
                    } else {
                        second[i] = p + 1;
                    }
                    ...
                    pos[i] = p - 1;
                    if (p == 1 || abs(A[p] - B[i]) <= abs(A[p - 2] - B[i])) {
                        second[i] = p;
                    } else {
                        second[i] = p - 2;
                    }
            ...
            absv.set(i, abs(A[pos[i]] - B[i]));
            second[i] = abs(A[second[i]] - B[i]);
            posm[pos[i]].pb(i);
        }
        ...
        rep(i, N) {
            ...
            ll bmin = 0, bmax_exist = false;
            if (i != 0) {
                if (B[0] <= A[i] - A[i - 1]) bmax_exist = true;
                bmin = lower_bound(all(B), A[i] + A[i - 1]) - B.begin();
            }
            if (bmax_exist) {
                if (i == N - 1 || A[i] <= A[i + 1] - B[0]) bob = true;
            }
            if (bmin < N) {
                // vll dbg;
                // rep(i, N) dbg.pb(absv.get(i));
                // debug(dbg);
                if (i == N - 1 || A[i] <= absv.prod(bmin, N)) bob = true;
            }
            repi(j, posm[i]) {
                absv.set(j, tmp.front());
                tmp.pop();
            }
            if (!bob) {
                // debug(i);
