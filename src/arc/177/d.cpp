...
void solve() {
    ...
    vm pow2(N + 1);
    pow2[0] = 1;
    rrep(i, N) pow2[i] = pow2[i - 1] * 2;

    vpll p;
    rep(i, N) p.eb(i, X[i]);
    sort(all(p), [](const pll &a, const pll &b) { return a.second < b.second; });

    vll p_inv(N);
    rep(i, N) p_inv[p[i].first] = i;

    vll cluster{0}, cluster_num, cluster_size;
    rep(i, N - 1) {
        cluster_num.pb(cluster.size() - 1);
        if (p[i].second + H < p[i + 1].second) {
            cluster.pb(i + 1);
        }
    }
    cluster_num.pb(cluster.size() - 1);
    rep(i, cluster.size() - 1) {
        cluster_size.pb(cluster[i + 1] - cluster[i]);
    }
    cluster_size.pb(N - cluster.back());

    // i と同じ cluster で，左/右にある i より若いものの数
    vll left(N, -1), right(N, -1);
    rep(i, cluster.size()) {
        stack<ll> l, r;
        rep(j, cluster_size[i]) {
            ll v = p[cluster[i] + j].first;
            while (!r.empty() && r.top() > v) r.pop();
            r.push(v);
            left[v] = r.size() - 1;
        }
        repd(j, cluster_size[i]) {
            ll v = p[cluster[i] + j].first;
            while (!l.empty() && l.top() > v) l.pop();
            l.push(v);
            right[v] = l.size() - 1;
        }
    }
    ...
    vm ans;
    segtree<mint, op, e> cl_pr(cluster.size());
    rep(i, cluster.size()) cl_pr.set(i, 0);
    rep(i, N) {
        ll num = cluster_num[p_inv[i]];
        mint pr = 1;
        if (cluster_size[num] == 1) {
        } else if (p_inv[i] == N - 1 || cluster_num[p_inv[i] + 1] != num) {
            if (p[p_inv[i] - 1].first > i) {
                pr /= 2;
            }
        } else if (p_inv[i] == 0 || cluster_num[p_inv[i] - 1] != num) {
            if (p[p_inv[i] + 1].first > i) {
                pr /= 2;
            }
        } else {
            ...
            } else if (p[p_inv[i] + 1].first > i) {
                pr /= 2;
            }
        }
        pr /= pow2[left[i]];
        pr /= pow2[right[i]];
        ans.pb(pr * cl_pr.prod(0, num) * cl_pr.prod(num + 1, cluster.size()));
        cl_pr.set(num, cl_pr.get(num) + pr);
    }

    debug(accumulate(all(ans), (mint)0));
