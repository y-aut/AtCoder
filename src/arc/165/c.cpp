...
bool calc(vvpll &wedges, ll shortest, ll x) {
    if (shortest < x) return false;
    ll N = wedges.size();
    vvll edges(N, vll());
    rep(i, N) {
        repi(p, wedges[i]) {
            if (p.second < x) edges[i].pb(p.first);
        }
    }
    if (!Bipartite(edges).is_bipartite()) return false;
    return true;
}
...
int main() {
    ...
    vvpll wedges(N, vpll());
    vpll shorter(N, pll(LINF, LINF));
    repi(e, ABW) {
        wedges[e[0]].eb(e[1], e[2]);
        wedges[e[1]].eb(e[0], e[2]);
        rep(i, 2) {
            ll v = e[i];
            ll w = e[2];
            if (w <= shorter[v].first) {
                shorter[v].second = shorter[v].first;
                shorter[v].first = w;
            } else if (w <= shorter[v].second) {
                shorter[v].second = w;
            }
        }
    }
    ll shortest = LINF;
    repi(i, shorter) chmin(shortest, i.first + i.second);

    ll upper = LINF, lower = 1;
    while (true) {
        if (lower + 1 >= upper) {
            print(lower);
            return 0;
