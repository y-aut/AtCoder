...
ll match_pre(vll &P, ll index, set<ll> &s) {
    ll ans = 0;
    repi(i, s) {
        if (P[index + ans] == i) ans++;
        else break;
    }
    return ans;
}
...
int main() {
    ...
    ll cnt = match_pre(P, 0, s);
    if (cnt == K) EXIT(print(P));
    ...
    rep(i, K, N) {
        ...
        if (cnt > 0) {
            ...
            index = i - K + 1;
            cnt = match_pre(P, index, s);
            if (cnt == K) EXIT(print(P));
        }
    }

    vll r;
    rep(i, index, index + K) r.pb(P[i]);
    sort(all(r));
    repi(i, r) {
        P[index++] = i;
    }
