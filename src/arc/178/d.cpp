...
void solve() {
    ...
    repd(i, N) {
        if (As.count(i)) {
            while (!d.empty() && d.front() >= i) l++, d.pop_front();
            while (!d.empty() && d.back() >= i) r++, d.pop_back();
            continue;
        }
        if (i == 0 && !As.count(1)) {
            rep(j, N) rep(k, N) dp[j][k] *= l + r + 1;
            break;
        }
        vector nxt(N + 1, vector(N + 1, mint(0)));
        vector ladd(N + 1, vector(N + 1, mint(0)));
        vector radd(N + 1, vector(N + 1, mint(0)));
        rep(j, N) rep(k, N) if (dp[j][k] != 0) {
            ladd[0][k + r] += dp[j][k];
            ladd[j + l + 1][k + r] -= dp[j][k];
            radd[j + l][0] += dp[j][k];
            radd[j + l][k + r + 1] -= dp[j][k];
        }
        rep(j, N) rep(k, N) {
            nxt[j][k] += ladd[j][k] + radd[j][k];
            ladd[j + 1][k] += ladd[j][k];
            radd[j][k + 1] += radd[j][k];
            if (i == 0) nxt[j][k] -= dp[j][k];
        }
        dp = move(nxt);
        if (i != 1) l = r = 0;
        // debug(i);
