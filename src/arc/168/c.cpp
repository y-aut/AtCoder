...
bool check(vll &cnt, vvll &c) {
    rep(i, 3) rep(j, 3) if (c[i][j] < 0) return false;
    rep(i, 3) {
        ll s = 0;
        rep(j, 3) s += c[i][j];
...
void solve() {
    ...
    Binomial bi(N + 10, MOD2);

    vll cnt(3);
    repi(c, S) cnt[c - 'A']++;

    mint ans = 0;
    rep(i, K + 1) rep(j, K + 1) rep(k, K + 1) rep(l, K + 1) rep(m, 2) {
        ll ops = i * 2 + j + k + l;
        if (ops > K) continue;
        if (i == 0 && m == 1) continue;
        vvll c(3, vll(3));
        ...
        c[2][0] += l;
        c[0][2] += l;
        rep(p, 3) c[p][p] = cnt[p] - c[p][0] - c[p][1] - c[p][2];
        if (!check(cnt, c)) continue;
        mint tmp = 1;
        rep(p, 3) tmp *= bi.nCk(cnt[p], c[p][0]) * bi.nCk(cnt[p] - c[p][0], c[p][1]);
        ans += tmp;
    }
    print(ans);
}
