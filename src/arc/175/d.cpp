...
void solve() {
    ...
    auto dfs = [&](auto rc, ll v) -> void {
        ...
        repi(i, tree.get_children(v)) {
            rc(rc, i);
            s += dsum[i] + tree.get_partial_size(i);
        }
        dsum[v] = s;
    };
    dfs(dfs, 0);
    ...
    vll ans(N, -1);
    ll cur = 0;
    auto set_rev = [&](ll v) -> void {
        queue<ll> q;
        q.push(v);
        cur += tree.get_partial_size(v);
        ll last = cur;
        while (!q.empty()) {
            auto u = q.front();
            q.pop();
            ans[u] = --last;
            repi(i, tree.get_children(u)) {
                q.push(i);
            }
        }
    };
    K = dsum[0] - K;
    auto dfs2 = [&](auto rc, ll v) -> void {
        if (K >= dsum[v]) {
            K -= dsum[v];
            set_rev(v);
            return;
        ...
        repi(i, tree.get_children(v)) {
            bool flg = false;
            if (K >= tree.get_partial_size(i)) {
                K -= tree.get_partial_size(i);
                ans[v] = --cur + tree.get_partial_size(i);
                flg = true;
            }
            rc(rc, i);
            if (flg) cur++;
