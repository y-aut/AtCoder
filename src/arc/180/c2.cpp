...
void solve() {
    LL(N);
    VLL(A, N);
    vector<um<ll, mint>> dp(N);
    rep(i, N) {
        rep(j, i) if (A[j] != 0) dp[i][A[j]] = 1;
        rep(j, i) repi(v, cnt, dp[j]) {
            if (A[j] + v) dp[i][A[j] + v] += cnt;
            else {
                usll s;
                rep(k, j + 1, i) if (A[k] != 0) s.insert(A[k]);
                repi(k, s) dp[i][k] += cnt;
            }
        }
    }
    mint ans = 1;
    rep(i, N) repi(v, cnt, dp[i]) ans += cnt;
    print(ans);
}
