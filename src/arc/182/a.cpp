...
void solve() {
    ...
    rep(i, Q) {
        rep(j, i) {
            if (PV[j].second > PV[i].second) {
                if (PV[j].first == PV[i].first) {
                    print(0);
                    return;
                }
                if (num[j] != -1 && num[j] != PV[j].first > PV[i].first) {
                    print(0);
                    return;
                }
                num[j] = PV[j].first > PV[i].first;
                if (num[i] != -1 && num[i] != PV[i].first > PV[j].first) {
                    print(0);
                    return;
                }
                num[i] = PV[i].first > PV[j].first;
            }
        }
    }
    repi(i, num) if (i == -1) ans *= 2;
    print(ans);
}
